...
public class BackupRestoreController {
    ...
    private void saveSettings() {
        try (Connection connection = DatabaseManager.getConnection();
            ...
            statement.setString(1, "backup.schedule");
            statement.setString(2, cmbSchedule.getValue());
            statement.executeUpdate();

            statement.setString(1, "backup.retention");
            statement.setString(2, String.valueOf(spRetention.getValue()));
            statement.executeUpdate();

            NotificationService.add("Backup preferences saved.");
            updateScheduleSummary();
            int removed = BackupManager.applyRetention(spRetention.getValue());
            refresh();
    ...
    private void loadSettings() {
        try (Connection connection = DatabaseManager.getConnection();
            ...
            while (result.next()) {
                if (result.getString(1).equals("backup.schedule")) {
                    cmbSchedule.setValue(result.getString(2));
                } else if (result.getString(1).equals("backup.retention")) {
                    spRetention.getValueFactory().setValue(
