...
public class InventoryController {
 ...
 private void configureTableInteractions(){
  tableItems.setRowFactory(table->{
   ...
   MenuItem add=new MenuItem("＋ Add Item"),edit=new MenuItem("✎ Edit Item"),adjust=new MenuItem("↕ Adjust Stock"),history=new MenuItem("◷ View Stock History"),delete=new MenuItem("🗑 Delete Item");
...
private void setupActions(){colActions.setCellFactory(c->new TableCell<>(){final MenuButton m=new MenuButton("Actions");{MenuItem a=new MenuItem("Adjust Stock",IconFactory.icon("stock")),h=new MenuItem("View History",IconFactory.icon("history"));a.setOnAction(e->adjust(getTableView().getItems().get(getIndex())));h.setOnAction(e->history(getTableView().getItems().get(getIndex())));m.getItems().addAll(a,h);m.getStyleClass().add("row-actions");}protected void updateItem(Void v,boolean e){super.updateItem(v,e);setGraphic(e?null:m);}});}
 ...
 @FXML private void adjustStock(){Dialog<Item>pick=new Dialog<>();pick.setTitle("Select Item");ComboBox<Item>box=new ComboBox<>(FXCollections.observableArrayList(all));box.setConverter(new javafx.util.StringConverter<>(){public String toString(Item i){return i==null?"":i.getItemCode()+" - "+i.getDescription();}public Item fromString(String s){return null;}});pick.getDialogPane().setContent(box);pick.getDialogPane().getButtonTypes().addAll(ButtonType.OK,ButtonType.CANCEL);pick.setResultConverter(b->b==ButtonType.OK?box.getValue():null);pick.showAndWait().ifPresent(this::adjust);}@FXML private void adjustSelectedStock(){if(selected==null)warn("Select an item first.");else adjust(selected);}
