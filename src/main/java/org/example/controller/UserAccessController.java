...
public class UserAccessController {
    ...
    private void resetPassword(UserRow row){if(row==null){warning("Select a user first.");return;}TextInputDialog d=new TextInputDialog();d.setTitle("Reset Password");d.setHeaderText("Set a temporary password for "+row.user.get());d.setContentText("Temporary password:");d.showAndWait().map(String::trim).filter(x->x.length()>=6).ifPresent(password->{try(Connection c=DatabaseManager.getConnection();PreparedStatement p=c.prepareStatement("UPDATE users SET password=?,failed_attempts=0,locked=0 WHERE id=?")){p.setString(1,password);p.setInt(2,row.id);p.executeUpdate();audit(row.id,"PASSWORD_RESET",row.user.get());NotificationService.add("Password reset for "+row.user.get()+".");refresh();}catch(Exception e){error("Password could not be reset",e);}});}
    ...
    private MenuItem mi(String text,String icon,javafx.event.EventHandler<javafx.event.ActionEvent>handler){MenuItem i=new MenuItem(text,IconFactory.icon(icon));i.setOnAction(handler);return i;}
    ...
    private boolean confirm(String text){return new Alert(Alert.AlertType.CONFIRMATION,text,ButtonType.YES,ButtonType.NO).showAndWait().orElse(ButtonType.NO)==ButtonType.YES;}
    private void warning(String message){new Alert(Alert.AlertType.WARNING,message).showAndWait();} private void error(String message,Exception e){e.printStackTrace();new Alert(Alert.AlertType.ERROR,message+".\n\n"+e.getMessage()).showAndWait();}
