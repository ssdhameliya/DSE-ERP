...
public class SalesListController implements ScreenLifecycle {
    ...
    private void createReminder(Sales sale){DatePicker due=new DatePicker(sale.getDueDate()==null?LocalDate.now().plusDays(1):sale.getDueDate());TextInputDialog dialog=new OwnedTextInputDialog("Payment reminder for "+sale.getInvoiceNo());dialog.setTitle("Create Reminder");dialog.setHeaderText("Reminder date: "+due.getValue());dialog.setContentText("Reminder text:");dialog.showAndWait().ifPresent(text->{try{String priority=sale.getDueDate()!=null&&sale.getDueDate().isBefore(LocalDate.now())?"URGENT":"NORMAL";String notes="Customer: "+sale.getCustomer().getName()+"; Balance: "+money(sale.getBalanceAmount());if(ConfigManager.isApiDataEnabled()){new InsightsApiClient().saveReminder(new InsightsApiClient.ReminderDto(null,text,sale.getInvoiceNo(),due.getValue().toString(),priority,notes,"OPEN",org.example.service.SessionService.current()==null?"System":org.example.service.SessionService.current().getUsername(),null));}else{try(Connection c=DatabaseManager.getConnection();PreparedStatement p=c.prepareStatement("INSERT INTO reminder_register(title,reference_no,due_date,priority,notes,status) VALUES(?,?,?,?,?,'OPEN')")){p.setString(1,text);p.setString(2,sale.getInvoiceNo());p.setString(3,due.getValue().toString());p.setString(4,priority);p.setString(5,notes);p.executeUpdate();}}NotificationService.add("Reminder created for "+sale.getInvoiceNo());info("Reminder created.");}catch(Exception e){error(e);}});}
