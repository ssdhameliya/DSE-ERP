...
public class PurchaseReturnsController {
    ...
    @FXML private void load() {
        ...
        String sql = "SELECT return_no,MAX(return_date),MAX(invoice_no),MAX(COALESCE(p.name,'')),SUM(amount),SUM(COALESCE(refund_amount,0))," +
            "CASE WHEN MAX(status)='CANCELLED' THEN 'CANCELLED' WHEN SUM(COALESCE(refund_amount,0))>=SUM(amount) AND SUM(amount)>0 THEN 'COMPLETED' WHEN SUM(COALESCE(refund_amount,0))>0 THEN 'PARTIAL' ELSE MAX(COALESCE(status,'PENDING')) END," +
            "CASE WHEN SUM(COALESCE(refund_amount,0))<=0 THEN 'PENDING' WHEN SUM(COALESCE(refund_amount,0))>=SUM(amount) THEN 'REFUNDED' ELSE 'PARTIAL' END " +
            "FROM return_register rr LEFT JOIN party_master p ON p.id=rr.party_id WHERE return_type='PURCHASE RETURN' GROUP BY return_no ORDER BY MAX(return_date) DESC";
        try (Connection c = DatabaseManager.getConnection(); Statement s = c.createStatement(); ResultSet r = s.executeQuery(sql)) {
            while (r.next()) rows.add(new Row(r.getString(1), r.getString(2), r.getString(3), r.getString(4), r.getDouble(5), r.getDouble(6), safe(r.getString(7)), safe(r.getString(8))));
        } catch (Exception e) { error(e); }
        all = rows;
        supplier.setItems(FXCollections.observableArrayList("All Suppliers")); supplier.getItems().addAll(rows.stream().map(Row::supplier).filter(x -> !x.isBlank()).distinct().sorted().toList()); if (supplier.getValue() == null) supplier.setValue("All Suppliers");
        ...
        double sum = rows.stream().mapToDouble(Row::total).sum(), refunded = rows.stream().mapToDouble(Row::refund).sum();
        total.setText(money(sum)); count.setText(String.valueOf(rows.size()));
        int thisMonth = (int) rows.stream().filter(row -> parse(row.date()).getYear() == LocalDate.now().getYear()
            && parse(row.date()).getMonthValue() == LocalDate.now().getMonthValue()).count();
        if (monthCount != null) monthCount.setText(String.valueOf(thisMonth));
        refund.setText(money(refunded)); average.setText(money(rows.isEmpty() ? 0 : sum / rows.size())); filter();
    }
