...
public class BankExpenseController implements ScreenLifecycle {
    ...
    @FXML public void initialize() {
        if (!ConfigManager.isApiDataEnabled()) ensureFinanceColumns();
    ...
    private void loadMetrics() {
        if (ConfigManager.isApiDataEnabled()) {
            try {
                ...
                if (mode == Mode.BANK) {
                    setKpi(kpi1Label,kpi1Value,kpi1Note,"Bank Balance",money(m.bankBalance()),"In all accounts");
                    setKpi(kpi2Label,kpi2Value,kpi2Note,"Total Bank Entries",String.valueOf(m.bankEntries()),"This month");
                    setKpi(kpi3Label,kpi3Value,kpi3Note,"Total Credits",money(m.credits()),"All time");
                    setKpi(kpi4Label,kpi4Value,kpi4Note,"Total Debits",money(m.debits()),"All time");
                } else {
                    setKpi(kpi1Label,kpi1Value,kpi1Note,"Total Expenses (This Month)",money(m.expenseMonth()),m.expenseEntries()+" entries");
                    setKpi(kpi2Label,kpi2Value,kpi2Note,"Total Expenses (This Year)",money(m.expenseYear()),"Year to date");
                    setKpi(kpi3Label,kpi3Value,kpi3Note,"Top Expense Category",m.topExpenseCategory(),money(m.topExpenseAmount()));
                    setKpi(kpi4Label,kpi4Value,kpi4Note,"Pending Reconcile",m.pendingReconcile()+" entries",money(m.pendingReconcileAmount()));
                }
        ...
        try {
            ...
            if (ConfigManager.isApiDataEnabled()) {
                if (mode==Mode.EXPENSE && reconciliationStatementId!=null && editingId==null) {
                    ...
                    OperationsApiClient.FinanceEntry dto = new OperationsApiClient.FinanceEntry(editingId, editingId==null?financeService.nextVoucher():null, rawType, entryDate.getValue().toString(), null, category, referenceNo.getText().trim(), value, paymentMode.getValue(), description.getText().trim(), account, selectedBill==null?null:selectedBill.getAbsolutePath(), false);
            ...
            boolean updated = editingId != null;
            String entity = mode==Mode.BANK ? "Bank Entry" : "Expense Entry";
            NotificationService.add(entity+" "+(updated?"updated":"recorded")+".");
            info(entity+(updated?" Updated":" Saved"), entity+" has been "+(updated?"updated":"saved")+" successfully.");
    ...
    private void reloadRows(){
        ...
        if (ConfigManager.isApiDataEnabled()) {
            ...
            String sql= mode==Mode.BANK ? "SELECT * FROM finance_register WHERE UPPER(voucher_type) IN ('BANK DEPOSIT','BANK WITHDRAWAL') ORDER BY voucher_date DESC,id DESC" : "SELECT * FROM finance_register WHERE UPPER(voucher_type)='EXPENSE' ORDER BY voucher_date DESC,id DESC";
            try(Connection c=DatabaseManager.getConnection(); Statement st=c.createStatement(); ResultSet r=st.executeQuery(sql)){ while(r.next()){ String raw=r.getString("voucher_type"); String type=mode==Mode.BANK?(raw.toUpperCase(Locale.ROOT).contains("DEPOSIT")?"Deposit":"Withdrawal"):safe(r.getString("category"),"Other"); EntryRow row=new EntryRow(r.getInt("id"),r.getString("voucher_date"),type,safe(r.getString("notes"),""),safe(r.getString("account_name"),""),safe(r.getString("payment_mode"),""),safe(r.getString("reference_no"),""),r.getDouble("amount"),raw,null,"",null,""); if(!matchesPeriod(row.date.get(),period))continue; if(filter!=null&&!filter.startsWith("All")&&!filter.equalsIgnoreCase(type))continue; String hay=(type+" "+row.description.get()+" "+row.account.get()+" "+row.reference.get()).toLowerCase(Locale.ROOT); if(!q.isEmpty()&&!hay.contains(q))continue; filtered.add(row); }}catch(Exception e){error("Unable to load entries: "+e.getMessage());}
        }
        renderPage();
    }
