...
public class BankExpenseController {
    ...
    private void addColumn(String name,String ddl){ try(Connection c=DatabaseManager.getConnection();Statement s=c.createStatement();ResultSet r=s.executeQuery("PRAGMA table_info(finance_register)")){while(r.next())if(name.equalsIgnoreCase(r.getString("name")))return;}catch(Exception ignored){} try(Connection c=DatabaseManager.getConnection();Statement s=c.createStatement()){s.execute("ALTER TABLE finance_register ADD COLUMN "+name+" "+ddl);}catch(Exception ignored){} }
