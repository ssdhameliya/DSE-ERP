...
public class RecordPaymentController {
    ...
    @FXML private void save() {
        try {
            ...
            Path storedProof=selectedAttachment==null?null:storeAttachment(selectedAttachment);
            try(Connection c=DatabaseManager.getConnection()) {
                c.setAutoCommit(false);
                try(PreparedStatement insert=c.prepareStatement(
                        "INSERT INTO payment_record(document_type,document_id,payment_date,amount,payment_mode,reference_no,notes,created_by,received_from,payment_type,attachment_path) VALUES('SALE',?,?,?,?,?,?,?,?,?,?)");
                    PreparedStatement update=c.prepareStatement(
                        "UPDATE sales_header SET paid_amount=COALESCE(paid_amount,0)+?, payment_status=CASE WHEN COALESCE(paid_amount,0)+?>=total_amount THEN 'PAID' ELSE 'PARTIAL' END WHERE id=?")) {
                    insert.setInt(1,sale.getId()); insert.setString(2,paymentDate.getValue().toString()); insert.setDouble(3,value);
                    insert.setString(4,mode.getValue()); insert.setString(5,reference.getText().trim()); insert.setString(6,notes.getText().trim());
                    insert.setString(7,"Admin"); insert.setString(8,receivedFrom.getText().trim());
                    insert.setString(9,fullPayment.isSelected()?"FULL":advancePayment.isSelected()?"ADVANCE":"PARTIAL");
                    insert.setString(10,storedProof==null?null:storedProof.toString());
                    insert.executeUpdate();
                    update.setDouble(1,value); update.setDouble(2,value); update.setInt(3,sale.getId()); update.executeUpdate();
                    c.commit();
                } catch(Exception e){ c.rollback(); if(storedProof!=null) Files.deleteIfExists(storedProof); throw e; }
            }
    ...
    private void loadHistory() {
        allPayments.clear();
        try(Connection c=DatabaseManager.getConnection();
            PreparedStatement s=c.prepareStatement("SELECT id,payment_date,reference_no,payment_mode,amount,notes,received_from,attachment_path FROM payment_record WHERE document_type='SALE' AND document_id=? ORDER BY payment_date DESC,id DESC")) {
            s.setInt(1,sale.getId());
            try(ResultSet r=s.executeQuery()) {
                while(r.next()) allPayments.add(new PaymentRow(r.getInt("id"),r.getString("payment_date"),safe(r.getString("reference_no")),
                    safeOr(r.getString("received_from"),sale.getCustomer().getName()),safe(r.getString("payment_mode")),r.getDouble("amount"),
                    "Recorded",safe(r.getString("notes")),safe(r.getString("attachment_path"))));
            }
        } catch(Exception e){ new OwnedAlert(Alert.AlertType.ERROR,e.getMessage()).showAndWait(); }
