...
public final class ReturnEditorService {
    ...
    private static List<ReturnLine> loadReturnableLines(Type type, String invoiceNo,
                                                         List<InvoiceItem> items) {
        List<ReturnLine> rows = new ArrayList<>();
        String sql = "SELECT COALESCE(SUM(quantity),0) FROM return_register " +
            "WHERE return_type=? AND invoice_no=? AND item_code=? " +
        ...
        try (Connection connection = DatabaseManager.getConnection();
            ...
            for (InvoiceItem item : items) {
                ...
                statement.setString(3, item.code());
                double returned;
                try (ResultSet result = statement.executeQuery()) {
                    returned = result.next() ? result.getDouble(1) : 0;
                }
                rows.add(new ReturnLine(item, returned));
            }
        } catch (Exception exception) {
            throw new IllegalStateException("Previously returned quantities could not be loaded", exception);
        }
        return rows;
    }
    ...
    private static String save(Type type, String invoiceNo, int partyId,
                               LocalDate returnDate, List<ReturnLine> rows) throws Exception {
        String returnNo = type.numberPrefix + System.currentTimeMillis();
        String insertSql = "INSERT INTO return_register" +
            "(return_no,return_type,return_date,invoice_no,party_id,item_code,quantity," +
            "amount,reason,status,refund_amount,refund_status) " +
            "VALUES(?,?,?,?,?,?,?,?,?,'PENDING',0,'PENDING')";
        String stockSql = type == Type.SALES
        ...
        try (Connection connection = DatabaseManager.getConnection()) {
            ...
            try (PreparedStatement insert = connection.prepareStatement(insertSql);
                ...
                for (ReturnLine row : rows) {
                    insert.setString(1, returnNo);
                    insert.setString(2, type.databaseValue);
                    insert.setString(3, returnDate.toString());
                    insert.setString(4, invoiceNo);
                    insert.setInt(5, partyId);
                    insert.setString(6, row.code());
                    insert.setDouble(7, row.returnQuantity());
                    insert.setDouble(8, row.returnAmount());
                    insert.setString(9, row.reason());
                    insert.addBatch();

                    stock.setDouble(1, row.returnQuantity());
                    stock.setString(2, row.code());
                    if (type == Type.PURCHASE) stock.setDouble(3, row.returnQuantity());
                    stock.addBatch();
                }
                insert.executeBatch();
                int[] stockResults = stock.executeBatch();
                if (type == Type.PURCHASE) {
                    for (int result : stockResults) {
                        if (result == 0) {
                            throw new IllegalStateException("Available stock changed while creating the return. Refresh and try again.");
                        }
                    }
                }
                connection.commit();
