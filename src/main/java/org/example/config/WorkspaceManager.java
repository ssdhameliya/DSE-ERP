...
public final class WorkspaceManager {
    ...
    public static synchronized void initialize() {
        try {
            ...
            Path legacy = detectLegacyWorkspace();
            if (legacy != null) {
                configure(legacy);
                organizeLegacyFiles(legacy);
            }
    ...
    /**
     * Schedules a workspace copy for the next application start, when SQLite is closed.
    ...
    private static void applyPendingMoveIfPresent() throws IOException {
        ...
        Path database = target.resolve("Database").resolve("JavaAppERP.db");
        Path legacyDatabase = target.resolve("JavaAppERP.db");
        if (!Files.exists(database) && Files.exists(legacyDatabase)) {
            Files.move(legacyDatabase, database, StandardCopyOption.REPLACE_EXISTING);
        }
        writePointer(target);
        Files.deleteIfExists(PENDING_MOVE_FILE);
    }
    ...
    private static Path detectLegacyWorkspace() {
        Path platform = POINTER_FOLDER;
        if (containsLegacyData(platform)) return platform;
        Path hidden = Path.of(System.getProperty("user.home"), ".dse-erp").toAbsolutePath().normalize();
        if (containsLegacyData(hidden)) return hidden;
        return null;
    }

    private static boolean containsLegacyData(Path folder) {
        return Files.isDirectory(folder) && (
                Files.isRegularFile(folder.resolve("JavaAppERP.db"))
                        || Files.isRegularFile(folder.resolve("config.properties"))
                        || Files.isRegularFile(folder.resolve("Database").resolve("JavaAppERP.db"))
                        || Files.isRegularFile(folder.resolve("Config").resolve("config.properties"))
        );
    }

    private static void organizeLegacyFiles(Path root) throws IOException {
        ...
        Path oldDatabase = root.resolve("JavaAppERP.db");
        Path newDatabase = root.resolve("Database").resolve("JavaAppERP.db");
        if (Files.isRegularFile(oldDatabase) && !Files.exists(newDatabase)) {
            Files.move(oldDatabase, newDatabase, StandardCopyOption.REPLACE_EXISTING);
        }
    }
