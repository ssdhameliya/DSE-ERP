...
6. The production job re-checks that the same version is healthy in UAT, configures the PROD private-update token, verifies the canonical artifact against the published release checksum, deploys it to PROD, verifies PROD health, explicitly promotes the same GitHub Release from prerelease to stable and Latest, waits for GitHub Latest convergence, and then verifies the stable private update gateway with bounded retries. No server rebuild occurs between UAT and PROD.
