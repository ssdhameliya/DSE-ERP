...
/**
 * PDF Studio V2 — object-driven WYSIWYG PDF template editor.
 *
...
public class PdfDesignerController implements ScreenLifecycle {
    ...
    @FXML private Button btnPreview, btnSaveDefault;
    ...
    public void initialize() {
        ...
        try {
            TemplateStorageService.migrateToStudioV2(template);
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                imageCache.computeIfAbsent(page, p -> {
                    try { return PdfImageExtractionService.extract(sourcePdf, p, WorkspaceManager.getTempFolder().resolve("pdf-studio-v2-images")); }
    ...
    private Node sourceImageNode(PdfImageRegion region) {
        StackPane hit = new StackPane(new Label("IMAGE"));
    ...
    private Node sourceVectorNode(PdfImageExtractionService.VectorRegion region) {
        StackPane hit = new StackPane(new Label(region.kind()));
    ...
    private Image imageFor(TemplateElement e) {
        try {
            Path p = e.getType()==ElementType.IMAGE_FIELD
                    ? (currentPreviewData == null ? null : currentPreviewData.image(e.getFieldKey()))
    ...
    private String displayText(TemplateElement e) {
        ...
        if (e.getType()==ElementType.BLOCK) return selectedIds.contains(e.getId()) ? "Block / Container" : "";
        if (e.getType()==ElementType.WHITEOUT) return "";
        String text = e.getText();
        if (currentPreviewData != null) text = resolveExpression(text, currentPreviewData);
        return text;
    }
    ...
    @FXML private void deleteSelected(){if(selectedIds.isEmpty()||previewMode)return;checkpoint();Set<String> ids=idsWithDescendants(selectedIds);List<TemplateElement> list=template.getElements().stream().filter(e->!ids.contains(e.getId())).collect(Collectors.toCollection(ArrayList::new));template.setElements(list);selectedIds.clear();autosave();clearInspector();renderCanvas();}
    ...
    @FXML private void saveDraft(){if(template==null)return;try{TemplateStorageService.save(template);lblSaveState.setText("Saved ✓");ModernDialog.success(root,"Template saved",template.getName()+" was saved.");}catch(Exception e){ModernDialog.error(root,"Save failed","PDF Studio",rootMessage(e));}}
    @FXML private void saveAndDefault(){if(template==null)return;try{TemplateStorageService.save(template);if(template.getDocumentType().isGeneral()||!DocumentFlowRegistry.isAutomatic(template.getDocumentType())){ModernDialog.success(root,"Template saved",template.getName()+" was saved as a design template.");return;}if(!ModernDialog.confirm(root,"Activate Default Template","Use "+template.getName()+" as the default "+template.getDocumentType().label()+"?","The existing built-in renderer remains the safety fallback if a custom render fails."))return;TemplateStorageService.activateAndSetDefault(template);refreshMeta();ModernDialog.success(root,"Default template updated",template.getName()+" is now the active default.");}catch(Exception e){ModernDialog.error(root,"Default could not be activated","PDF Studio",rootMessage(e));}}

    @FXML private void togglePreview(){if(template==null)return;if(previewMode){previewMode=false;previewPdf=null;btnPreview.setText("Preview");configurePages(sourcePageCount);clearSelection();renderCanvas();ensurePageObjects(pageIndex);return;}try{TemplateStorageService.save(template);previewPdf=WorkspaceManager.getTempFolder().resolve("pdf-studio-v2-preview-"+template.getId()+".pdf");PdfTemplateRenderer.render(template,previewData(),previewPdf);previewMode=true;btnPreview.setText("Back to Design");var size=PdfPreviewSupport.pageSize(previewPdf,0);configurePages(size.pageCount());clearSelection();renderCanvas();}catch(Exception e){previewMode=false;ModernDialog.error(root,"Preview failed","PDF Studio",rootMessage(e));}}

    @FXML private void exportPdf(){if(template==null)return;FileChooser chooser=new FileChooser();chooser.setTitle("Export PDF");chooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("PDF files","*.pdf"));chooser.setInitialFileName(template.getName().replaceAll("[^A-Za-z0-9._-]","-")+".pdf");var file=chooser.showSaveDialog(root.getScene().getWindow());if(file==null)return;try{TemplateStorageService.save(template);PdfTemplateRenderer.render(template,previewData(),file.toPath());ModernDialog.success(root,"PDF exported",file.getAbsolutePath());}catch(Exception e){ModernDialog.error(root,"Export failed","PDF Studio",rootMessage(e));}}
    ...
    private void autosave(){try{TemplateStorageService.save(template);lblSaveState.setText("Saved ✓");}catch(Exception e){lblSaveState.setText("Save failed");}refreshMeta();}

    // ---------------------------------------------------------------------
    // Utility
    // ---------------------------------------------------------------------

    private void refreshMeta(){if(template==null)return;lblTemplateName.setText(template.getName());lblTemplateMeta.setText(template.getDocumentType().label()+" • "+template.getStatus()+ (template.isDefaultTemplate()?" • DEFAULT":"") + " • Object-driven PDF Studio");}
    private void updateDefaultButton(){if(btnSaveDefault==null||template==null)return;btnSaveDefault.setText(DocumentFlowRegistry.isAutomatic(template.getDocumentType())?"Save & Set Default":"Save Template");}
    ...
    private String displayName(TemplateElement e){if(e==null)return "Object";return switch(e.getType()){case TEXT->"Text • "+abbreviate(e.getText(),28);case FIELD->"ERP Field • "+e.getFieldKey();case IMAGE->"Image";case IMAGE_FIELD->"ERP Image • "+e.getFieldKey();case BLOCK->"Block / Container";case RECTANGLE->"Rectangle";case WHITEOUT->"Source Mask";case LINE->"Line";case PATH->"Vector Path";case ITEM_TABLE->"Item Repeater";case CHARGE_TABLE->"Charge Repeater";};}
