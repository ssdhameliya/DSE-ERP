...
public final class OperationsApiClient {
    ...
    public SalesPage salesPage(int page,int size,String q,String invoice,String customer,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,Double minAmount,Double maxAmount){
        String path="/api/operations/sales/page?page="+Math.max(0,page)+"&size="+Math.max(10,Math.min(size,200))+"&q="+enc(q)+"&invoice="+enc(invoice)+"&customer="+enc(customer)+"&from="+enc(str(from))+"&to="+enc(str(to))+"&paymentStatus="+enc(paymentStatus)+"&due="+enc(due)+"&mail="+enc(mail)+"&whatsapp="+enc(whatsapp)+"&invoiceType="+enc(invoiceType);
    ...
    public PurchasePage purchasesPage(int page,int size,String q,String supplier,LocalDate from,LocalDate to,String paymentStatus,String mail){PurchasePageDto d=get("/api/operations/purchases/page?page="+Math.max(0,page)+"&size="+Math.max(10,Math.min(size,200))+"&q="+enc(q)+"&supplier="+enc(supplier)+"&from="+enc(str(from))+"&to="+enc(str(to))+"&paymentStatus="+enc(paymentStatus)+"&mail="+enc(mail),PurchasePageDto.class);return new PurchasePage(d.rows()==null?List.of():d.rows().stream().map(this::purchase).toList(),d.page(),d.size(),d.totalRows(),d.totalPages(),d.filteredTotals(),d.metrics(),d.suppliers()==null?List.of():d.suppliers());}
    ...
    public FinancePage financePage(int page,int size,String mode,String period,String type,String q){return get("/api/operations/finance/page?page="+Math.max(0,page)+"&size="+Math.max(10,Math.min(size,200))+"&mode="+enc(mode)+"&period="+enc(period)+"&type="+enc(type)+"&q="+enc(q),FinancePage.class);}
    ...
    private void applySaleIdentity(Sales target,SaleDto saved){if(target==null||saved==null)return;target.setId(n(saved.id));target.setRowVersion(saved.rowVersion);}
    private void applyPurchaseIdentity(Purchase target,PurchaseDto saved){if(target==null||saved==null)return;target.setId(n(saved.id));target.setRowVersion(saved.rowVersion);}
    ...
    private <T>T request(String method,String path,Object body,Class<T> cls,TypeReference<T> type){try{HttpRequest.Builder b=HttpRequest.newBuilder(URI.create(base+path)).timeout(Duration.ofSeconds(25)).header("Accept","application/json");org.example.api.ApiSession.authorize(b);if(body!=null){b.header("Content-Type","application/json");b.method(method,HttpRequest.BodyPublishers.ofString(json.writeValueAsString(body)));}else b.method(method,HttpRequest.BodyPublishers.noBody());HttpResponse<String> r=http.send(b.build(),HttpResponse.BodyHandlers.ofString());if(r.statusCode()<200||r.statusCode()>=300){if(r.statusCode()==409)throw new IllegalStateException(apiMessage(r.body(),"This record was changed by another user. Reload the latest version and try again."));throw new IllegalStateException("Operations API error ("+r.statusCode()+"): "+r.body());}return type!=null?json.readValue(r.body(),type):json.readValue(r.body(),cls);}catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException("Operations API request interrupted",e);}catch(IOException|IllegalArgumentException e){throw new IllegalStateException("Cannot reach operations server at "+base,e);}}
