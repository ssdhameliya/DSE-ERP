...
public final class MasterApiClient {
 ...
 private void applyPartyIdentity(Party p,PartyDto d){if(p==null||d==null)return;p.setId(n(d.id));p.setPartyCode(d.partyCode);p.setRowVersion(d.rowVersion());}
 private void applyItemIdentity(Item i,ItemDto d){if(i==null||d==null)return;i.setId(n(d.id));i.setItemCode(d.itemCode);i.setRowVersion(d.rowVersion());}
 private void applyLookupIdentity(Lookup l,LookupDto d){if(l==null||d==null)return;l.setId(n(d.id));l.setLookupCode(d.lookupCode);l.setRowVersion(d.rowVersion());}
 ...
 private <T>T request(String method,String path,Object body,Class<T> cls,TypeReference<T> type){try{HttpRequest.Builder b=HttpRequest.newBuilder(URI.create(base+path)).timeout(Duration.ofSeconds(15)).header("Accept","application/json");org.example.api.ApiSession.authorize(b); if(body!=null){b.header("Content-Type","application/json");String payload=json.writeValueAsString(body);b.method(method,HttpRequest.BodyPublishers.ofString(payload));}else b.method(method,HttpRequest.BodyPublishers.noBody()); HttpResponse<String> r=http.send(b.build(),HttpResponse.BodyHandlers.ofString()); if(r.statusCode()<200||r.statusCode()>=300){org.example.api.ApiRuntime.logHttpFailure("Master-data request",r.statusCode(),r.body());throw new IllegalStateException(org.example.api.ApiRuntime.userMessage("the master-data request",r.statusCode(),r.body()));} return type!=null?json.readValue(r.body(),type):json.readValue(r.body(),cls);}catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException("Master API request interrupted",e);}catch(IOException|IllegalArgumentException e){throw new IllegalStateException("Cannot reach master-data server at "+base,e);}}
