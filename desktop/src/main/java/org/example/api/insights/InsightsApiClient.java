...
public final class InsightsApiClient {
    ...
    public void reminderStatus(long id,String status,String snoozedUntil){postNoBody("/api/insights/reminders/"+id+"/status?status="+enc(status)+(snoozedUntil==null?"":"&snoozedUntil="+enc(snoozedUntil)));}
    public void deleteReminder(long id){delete("/api/insights/reminders/"+id);}
    ...
    private <T>T request(String method,String path,Object body,Class<T> cls,TypeReference<T> type){try{HttpRequest.Builder b=HttpRequest.newBuilder(URI.create(base+path)).timeout(Duration.ofSeconds(30)).header("Accept","application/json");org.example.api.ApiSession.authorize(b);if(body!=null){b.header("Content-Type","application/json");b.method(method,HttpRequest.BodyPublishers.ofString(json.writeValueAsString(body)));}else b.method(method,HttpRequest.BodyPublishers.noBody());HttpResponse<String> r=http.send(b.build(),HttpResponse.BodyHandlers.ofString());if(r.statusCode()<200||r.statusCode()>=300)throw new IllegalStateException(apiErrorMessage(r.statusCode(),r.body()));return type!=null?json.readValue(r.body(),type):json.readValue(r.body(),cls);}catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException("Insights API request interrupted",e);}catch(IOException|IllegalArgumentException e){throw new IllegalStateException("Cannot reach insights server at "+base,e);}}
