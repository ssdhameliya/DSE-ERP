...
public final class ServerResourceClient {
    ...
    public void put(String type,String key,String fileName,byte[] content){put(type,key,fileName,content,"");} public void put(String type,String key,String fileName,byte[] content,String expectedChecksum){try{String p=path(type,key)+"?filename="+enc(fileName)+"&contentType="+enc("application/zip")+"&expectedChecksum="+enc(expectedChecksum);sendBytes("PUT",p,content);}catch(Exception e){throw failure(e);}}
