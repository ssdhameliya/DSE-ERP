...
/** Sale-style Quotation create/edit workspace backed by Quotation-specific server lookups. */
public final class QuotationEditorController {
    ...
    private static final class CustomerChoice{final int id;final String code,name;CustomerChoice(Party p){id=p.getId();code=safe(p.getPartyCode());name=safe(p.getName());}String display(){return code.isBlank()?name:code+" - "+name;}@Override public String toString(){return display();}}
