...
public final class QuotationEditorController {
    ...
    @FXML private TableColumn<LineRow,String> colItem,colCode,colCategory,colHsn,colUnit;
    ...
    private long quotationRowVersion;
    ...
    private void configureTable(){
        ...
        colCategory.setCellValueFactory(v->v.getValue().category);
        colHsn.setCellValueFactory(v->v.getValue().hsn);
        colUnit.setCellValueFactory(v->v.getValue().unit);
    ...
    private void editLine(LineRow row){
        ...
        if(cached==null)cached=new ItemChoice(row.code.get(),row.description.get(),"",row.category.get(),row.hsn.get(),row.unit.get(),row.rate.get(),row.gst.get(),row.discount.get());
    ...
    @FXML private void chooseAttachment(){FileChooser chooser=new FileChooser();chooser.setTitle("Attach quotation document");chooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Documents","*.pdf","*.png","*.jpg","*.jpeg","*.doc","*.docx","*.xls","*.xlsx","*.csv","*.txt"));File file=chooser.showOpenDialog(tableLines.getScene().getWindow());if(file!=null){selectedAttachment=file.toPath();attachmentRemovalPending=false;dirty=true;org.example.navigation.UnsavedChangesManager.touch(tableLines);updateAttachmentLabel();}}
