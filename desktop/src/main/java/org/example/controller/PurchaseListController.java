...
public class PurchaseListController implements ScreenLifecycle{
 ...
 private void decorateDetailDrawer(){
  ...
  applyDrawerValueIcon(lblDetailInvoice,"document");applyDrawerValueIcon(lblDetailSupplier,"supplier");applyDrawerValueIcon(lblDetailContact,"user");applyDrawerValueIcon(lblDetailAmount,"currency");applyDrawerValueIcon(lblDetailPaid,"payment");applyDrawerValueIcon(lblDetailBalance,"balance");applyDrawerValueIcon(lblDetailDue,"calendar");applyDrawerValueIcon(lblDetailGst,"tax");applyDrawerValueIcon(lblDetailReference,"document");applyDrawerValueIcon(lblDetailPaymentTerms,"payment");applyDrawerValueIcon(lblDetailContactPerson,"user");applyDrawerValueIcon(lblDetailPhone,"phone");applyDrawerValueIcon(lblDetailEmail,"email");applyDrawerValueIcon(lblDetailGstin,"tax");
 }
 ...
 private void applyDrawerValueIcon(Label label,String semantic){if(label!=null&&label.getGraphic()==null){label.setGraphic(IconFactory.compactIcon(semantic,15));label.setGraphicTextGap(7);label.getProperties().put("erp-icon-preserve",true);}}
 private String drawerSemantic(String text){String t=s(text).toLowerCase(java.util.Locale.ROOT);if(t.contains("invoice"))return"document";if(t.contains("supplier"))return"supplier";if(t.contains("contact"))return"user";if(t.contains("phone"))return"phone";if(t.contains("email"))return"email";if(t.contains("gst"))return"tax";if(t.contains("balance"))return"balance";if(t.contains("paid")||t.contains("payment"))return"payment";if(t.contains("amount"))return"currency";if(t.contains("due"))return"calendar";if(t.contains("reference"))return"document";if(t.contains("pdf")||t.contains("print"))return"pdf";if(t.contains("close"))return"cancel";return null;}
 ...
 private void setupActions(){
  colActions.setCellFactory(c->new TableCell<>(){
   ...
   final MenuItem delete;
   {
    ...
    item("Print / Download PDF","print",e->pdf(row()));
    ...
    attach=item("Add / Replace Attachment","attachment",e->attach(row()));
    viewAttachment=item("View Attachment","view",e->viewAttachment(row()));
   ...
   private void updateAvailability(){
    ...
    boolean hasAttachment=!s(current.getAttachmentPath()).isBlank();
    ...
    attach.setDisable(deleted);attach.setText(hasAttachment?"Replace Attachment":"Add Attachment");
    viewAttachment.setDisable(!hasAttachment);removeAttachment.setDisable(!hasAttachment||deleted);
   }
 ...
 private void attach(Purchase p){FileChooser chooser=new FileChooser();chooser.setTitle("Choose purchase attachment");File file=chooser.showOpenDialog(tablePurchase.getScene().getWindow());if(file==null)return;try{String reference=support.uploadDocumentAttachment("PURCHASE",p.getId(),file.toPath());p.setAttachmentPath(reference);refresh();info("Purchase attachment saved.");}catch(Exception e){error(e);}}
 private void viewAttachment(Purchase p){try{SupportApiClient.DownloadedAttachment downloaded=support.documentAttachment("PURCHASE",p.getId());Path path=materializeAttachmentPreview(downloaded);if(path==null||!Files.isRegularFile(path))throw new FileNotFoundException("The stored purchase attachment is missing.");java.awt.Desktop.getDesktop().open(path.toFile());}catch(Exception e){error(e);}}
 private void removeAttachment(Purchase p){if(s(p.getAttachmentPath()).isBlank())return;if(new OwnedAlert(Alert.AlertType.CONFIRMATION,"Remove the attachment from "+p.getInvoiceNo()+"?",ButtonType.YES,ButtonType.NO).showAndWait().orElse(ButtonType.NO)!=ButtonType.YES)return;try{support.deleteDocumentAttachment("PURCHASE",p.getId());p.setAttachmentPath("");refresh();info("Purchase attachment removed.");}catch(Exception e){error(e);}}
