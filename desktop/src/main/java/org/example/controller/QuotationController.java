...
public class QuotationController implements org.example.navigation.ScreenLifecycle {
 ...
 @FXML private Button btnNewQuotation,btnReset,btnRefresh,btnSaveView,btnApplyFilters,btnExportExcel,btnExportPdf,btnTodayRange,btnYesterdayRange,btnSevenDaysRange,btnThirtyDaysRange,btnCustomRange,btnPreviewAttachment,btnRemoveAttachment;
 ...
 private boolean followMatch(QuoteRow q){String f=cmbFollowUp.getValue();if(f==null||f.equals("All"))return true;if(q.followDate==null)return f.equals("Not Set");long d=java.time.temporal.ChronoUnit.DAYS.between(BusinessClock.today(),q.followDate);return f.equals("Overdue")?d<0:f.equals("Today")?d==0:f.equals("Next 7 Days")&&d>=0&&d<=7;}
...
private void setupActions(){colActions.setCellFactory(c->new TableCell<>(){final MenuButton m=new MenuButton("Actions");{add("View","view",e->showDetails(row()));add("Edit","edit",e->edit(row()));add("View PDF","pdf",e->openPdf(row()));add("View / Download Excel","excel",e->openExcel(row()));add("Send Email","email",e->sendEmail(row()));add("Send WhatsApp","whatsapp",e->sendWhatsapp(row()));add("Create Follow Up","reminder",e->followUp(row()));add("Convert to Sale","sale",e->convert(row()));add("Duplicate","copy",e->duplicate(row()));add("Delete","delete",e->delete(row()));m.getStyleClass().add("row-actions");m.setGraphic(IconFactory.compactIcon("actions",16));m.setText("Actions");m.setContentDisplay(ContentDisplay.LEFT);m.setGraphicTextGap(6);m.setTooltip(new Tooltip("Actions"));m.getProperties().put("erp-icon-preserve",true);IconFactory.decorateActionMenu(m);}private QuoteRow row(){int index=getIndex();return index<0||index>=getTableView().getItems().size()?null:getTableView().getItems().get(index);}private void add(String t,String semantic,javafx.event.EventHandler<javafx.event.ActionEvent>h){MenuItem i=new MenuItem(t);i.setGraphic(IconFactory.compactIcon(semantic,16));i.setOnAction(h);m.getItems().add(i);}protected void updateItem(Void v,boolean e){super.updateItem(v,e);setGraphic(e?null:m);setAlignment(Pos.CENTER);}});}
 ...
 @FXML private void closeDetails(){selected=null;configureResponsiveTable();detailDrawer.setMinWidth(390);detailDrawer.setPrefWidth(410);detailDrawer.setMaxWidth(440);detailDrawer.setManaged(false);detailDrawer.setVisible(false);mainSplit.setDividerPositions(1);table.getSelectionModel().clearSelection();}private QuoteRow req(){if(selected==null)new OwnedAlert(Alert.AlertType.WARNING,"Select a quotation first.").showAndWait();return selected;}
 ...
 @FXML private void attachSelected(){QuoteRow q=req();if(q==null)return;FileChooser chooser=new FileChooser();chooser.setTitle("Attach quotation document");chooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Documents","*.pdf","*.png","*.jpg","*.jpeg","*.doc","*.docx","*.xls","*.xlsx","*.csv","*.txt"));File file=chooser.showOpenDialog(table.getScene().getWindow());if(file==null)return;try{q.attachment=supportApi.uploadDocumentAttachment("QUOTATION",q.id,file.toPath());updateAttachmentActions(q);log(q.id,"ATTACHMENT","Quotation attachment added/replaced");success("Quotation attachment saved.");}catch(Exception e){error(e);}}
 @FXML private void previewAttachmentSelected(){QuoteRow q=req();if(q==null)return;if(q.attachment==null||q.attachment.isBlank()){info("No quotation attachment is available.");updateAttachmentActions(q);return;}try{SupportApiClient.DownloadedAttachment d=supportApi.documentAttachment("QUOTATION",q.id);if(d==null||d.data()==null||d.data().length==0)throw new IllegalStateException("No quotation attachment is available.");Path folder=WorkspaceManager.getTempFolder().resolve("AttachmentPreview");Files.createDirectories(folder);String name=d.fileName()==null||d.fileName().isBlank()?"quotation-attachment":d.fileName().replaceAll("[^A-Za-z0-9._-]","_");Path target=folder.resolve(System.currentTimeMillis()+"-"+name);Files.write(target,d.data());target.toFile().deleteOnExit();java.awt.Desktop.getDesktop().open(target.toFile());}catch(Exception e){error(e);}}
 @FXML private void removeAttachmentSelected(){QuoteRow q=req();if(q==null)return;if(q.attachment==null||q.attachment.isBlank()){info("No quotation attachment is available.");updateAttachmentActions(q);return;}if(!confirm("Remove the quotation attachment?"))return;try{supportApi.deleteDocumentAttachment("QUOTATION",q.id);q.attachment="";updateAttachmentActions(q);log(q.id,"ATTACHMENT","Quotation attachment removed");success("Quotation attachment removed.");}catch(Exception e){error(e);}}
