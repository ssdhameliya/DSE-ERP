 ...
 * This page is an aggregation/navigation hub only: quotation, order, project,
 * invoice and payment records stay owned by their existing production modules.
 */
public class Customer360Controller {
    ...
    @FXML private Label lblOutstanding,lblOpenQuotes,lblOpenOrders,lblProjects,lblSales,lblLastPayment;
    @FXML private StackPane pageIcon,outstandingIcon,quotesIcon,ordersIcon,projectsIcon,salesIcon,paymentIcon;
    @FXML private TabPane tabs;
    @FXML private Tab tabOverview,tabContacts,tabQuotations,tabOrders,tabProjects,tabInvoices,tabPayments,tabNotes,tabDocuments;

    @FXML private TableView<Customer360ApiClient.QuotationRow> tblRecentQuotes,tblQuotations;
    @FXML private TableView<Customer360ApiClient.WorkflowRow> tblRecentOrders,tblOrders,tblProjects;
    @FXML private TableView<Customer360ApiClient.InvoiceRow> tblRecentInvoices,tblInvoices,tblDirectSales;
    ...
    @FXML private TableColumn<Customer360ApiClient.WorkflowRow,String> roNo,roDate,roAmount,roStatus,oNo,oPo,oDate,oProject,oAmount,oStatus,pNo,pName,pDate,pTarget,pAmount,pStatus;
    @FXML private TableColumn<Customer360ApiClient.InvoiceRow,String> riNo,riDate,riAmount,riOutstanding,iNo,iDate,iOrder,iProject,iAmount,iPaid,iOutstanding,iStatus,dsNo,dsDate,dsAmount,dsPaid,dsOutstanding,dsStatus;
    ...
    private void configureIcons(){
        icon(pageIcon,"customer",26);icon(outstandingIcon,"balance",22);icon(quotesIcon,"quotation",22);icon(ordersIcon,"sales-order",22);icon(projectsIcon,"project",22);icon(salesIcon,"amount",22);icon(paymentIcon,"payment",22);
    }
    ...
    private void configureTables(){
        ...
        roNo.setCellValueFactory(c->s(c.getValue().documentNo()));roDate.setCellValueFactory(c->s(c.getValue().documentDate()));roAmount.setCellValueFactory(c->s(money(c.getValue().totalAmount())));roStatus.setCellValueFactory(c->s(c.getValue().status()));
        ...
        oNo.setCellValueFactory(c->s(c.getValue().documentNo()));oPo.setCellValueFactory(c->s(c.getValue().customerPoNo()));oDate.setCellValueFactory(c->s(c.getValue().documentDate()));oProject.setCellValueFactory(c->s(c.getValue().projectNo()));oAmount.setCellValueFactory(c->s(money(c.getValue().totalAmount())));oStatus.setCellValueFactory(c->s(c.getValue().status()));
        pNo.setCellValueFactory(c->s(c.getValue().documentNo()));pName.setCellValueFactory(c->s(c.getValue().parentNo()));pDate.setCellValueFactory(c->s(c.getValue().documentDate()));pTarget.setCellValueFactory(c->s(c.getValue().expectedDate()));pAmount.setCellValueFactory(c->s(money(c.getValue().totalAmount())));pStatus.setCellValueFactory(c->s(c.getValue().status()));
        iNo.setCellValueFactory(c->s(c.getValue().invoiceNo()));iDate.setCellValueFactory(c->s(c.getValue().invoiceDate()));iOrder.setCellValueFactory(c->s(c.getValue().salesOrderNo()));iProject.setCellValueFactory(c->s(c.getValue().projectNo()));iAmount.setCellValueFactory(c->s(money(c.getValue().totalAmount())));iPaid.setCellValueFactory(c->s(money(c.getValue().paidAmount())));iOutstanding.setCellValueFactory(c->s(money(c.getValue().outstanding())));iStatus.setCellValueFactory(c->s(paymentStatus(c.getValue())));
        dsNo.setCellValueFactory(c->s(c.getValue().invoiceNo()));dsDate.setCellValueFactory(c->s(c.getValue().invoiceDate()));dsAmount.setCellValueFactory(c->s(money(c.getValue().totalAmount())));dsPaid.setCellValueFactory(c->s(money(c.getValue().paidAmount())));dsOutstanding.setCellValueFactory(c->s(money(c.getValue().outstanding())));dsStatus.setCellValueFactory(c->s(paymentStatus(c.getValue())));
        ...
        for(TableView<?> t:List.of(tblRecentQuotes,tblRecentOrders,tblRecentInvoices,tblContacts,tblQuotations,tblOrders,tblDirectSales,tblProjects,tblInvoices,tblPayments,tblNotes,tblDocuments))DynamicTableLayoutManager.install(t);
    }

    private void configureInteractions(){
        tblQuotations.setRowFactory(t->row(this::openQuotation));tblRecentQuotes.setRowFactory(t->row(this::openQuotation));
        tblOrders.setRowFactory(t->row(this::openWorkflow));tblRecentOrders.setRowFactory(t->row(this::openWorkflow));tblProjects.setRowFactory(t->row(this::openWorkflow));
        tblInvoices.setRowFactory(t->row(this::openInvoice));tblRecentInvoices.setRowFactory(t->row(this::openInvoice));tblDirectSales.setRowFactory(t->row(this::openInvoice));
    ...
    private void renderSummary(Customer360ApiClient.Summary s){
        ...
        lblOutstanding.setText(money(s.outstandingReceivable()));lblOpenQuotes.setText(money(s.openQuotationValue())+"  •  "+s.openQuotationCount()+" open");lblOpenOrders.setText(money(s.openSalesOrderValue())+"  •  "+s.openSalesOrderCount()+" open");lblProjects.setText(String.valueOf(s.activeProjectCount()));lblSales.setText(money(s.totalSales()));lblLastPayment.setText(money(s.lastPaymentAmount())+(safe(s.lastPaymentDate()).isBlank()?"":"  •  "+s.lastPaymentDate()));
        tblRecentQuotes.getItems().setAll(nz(s.recentQuotations()));tblRecentOrders.getItems().setAll(nz(s.recentSalesOrders()));tblRecentInvoices.getItems().setAll(nz(s.recentInvoices()));
    }
    ...
    private void loadTab(Tab tab){if(tab==null||loaded.contains(tab)||customer==null)return;loaded.add(tab);int id=customer.getId();
        ...
        else if(tab==tabOrders)loadOrderViews(id);
        else if(tab==tabProjects)load("customer-360-projects-"+id,()->api.projects(id),x->tblProjects.getItems().setAll(x));
    ...
    private void loadOrderViews(int id){
        UiTaskExecutor.submitLatest("customer-360-order-views-"+id,()->new OrderViews(api.salesOrders(id),api.directSales(id)),views->{
            tblOrders.getItems().setAll(views.workflow()==null?List.of():views.workflow());
            tblDirectSales.getItems().setAll(views.direct()==null?List.of():views.direct());
        },e->{loaded.remove(tabOrders);showError(e);});
    }
    private record OrderViews(List<Customer360ApiClient.WorkflowRow> workflow,List<Customer360ApiClient.InvoiceRow> direct){}
    ...
    private void openWorkflow(Customer360ApiClient.WorkflowRow r){if(r==null)return;WorkflowScreenContext.select(r.documentType(),r.id());NavigationManager.navigateOrReport("PROJECT".equals(r.documentType())?"/fxml/pages/Projects.fxml":"/fxml/pages/SalesOrders.fxml");}
