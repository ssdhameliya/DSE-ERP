...
public class OperationsController {
    ...
    private void loadReminders(){List<ReminderRow>x=new ArrayList<>();try{for(InsightsApiClient.ReminderDto r:insightsApi.reminders())x.add(new ReminderRow(r.id()==null?0L:r.id(),r.title(),r.referenceNo(),r.dueDate(),r.priority(),r.status()));}catch(Exception e){error(e);}reminderTable.getItems().setAll(x);}
