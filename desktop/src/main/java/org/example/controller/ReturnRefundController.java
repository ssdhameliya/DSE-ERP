...
public class ReturnRefundController implements ScreenLifecycle {
    ...
    @FXML private void save(){try{validate();double value=parse(amount.getText());String type=fullRefund.isSelected()?"FULL":"PARTIAL";String user=SessionService.current()==null?"User":safe(SessionService.current().getFullName());int id=api.recordRefund(details.no(),new ReturnApiClient.RefundCreateRequest(refundDate.getValue().toString(),value,mode.getValue(),safe(reference.getText()),bankAccount.isDisabled()?"":safe(bankAccount.getValue()),safe(refundedParty.getText()),safe(notes.getText()),type,user));invalidateReturnViews();if(pendingProof!=null){try{support.uploadReturnRefundAttachment(id,pendingProof);}catch(Exception upload){error(new IllegalStateException("Refund was saved, but the proof could not be uploaded. The refund remains recorded. "+message(upload),upload));pendingProof=null;load();return;}}NotificationService.add(details.no()+" refund recorded.");org.example.util.ToastManager.success(amount,"Refund recorded","Refund transaction saved successfully.");pendingProof=null;org.example.navigation.UnsavedChangesManager.markClean(amount);load();}catch(Exception e){error(e);}}
    private void invalidateReturnViews(){boolean sales=details!=null&&"SALES RETURN".equalsIgnoreCase(details.type());if(sales){ScreenRefreshPolicy.invalidate("sales-returns");ScreenRefreshPolicy.invalidate("sales-register");}else{ScreenRefreshPolicy.invalidate("purchase-returns");ScreenRefreshPolicy.invalidate("purchase-register");}}
    ...
    @FXML private void removeProof(){pendingProof=null;proofName.setText("No file selected");org.example.navigation.UnsavedChangesManager.touch(amount);}
