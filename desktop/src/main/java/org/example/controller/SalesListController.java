...
import org.example.util.PdfPreviewDialog;
...
public class SalesListController implements ScreenLifecycle {
    ...
    private String documentStatus(Sales sale){
        ...
        if (java.util.Set.of("CANCELLED","DELETED","RETURNED","DRAFT").contains(normalized)) return normalized;
    ...
    private void previewInvoicePdf(Sales sale){try{Sales full=service.getByInvoice(sale.getInvoiceNo());if(full==null)throw new IllegalStateException("Sales invoice not found. Refresh and try again.");Path pdf=InvoicePdfService.sales(full);PdfPreviewDialog.show(tableSales,pdf,"Sale Invoice • "+sale.getInvoiceNo());log("SALE",sale.getId(),"PDF_PREVIEWED",sale.getInvoiceNo());}catch(Exception e){error(e);}}
