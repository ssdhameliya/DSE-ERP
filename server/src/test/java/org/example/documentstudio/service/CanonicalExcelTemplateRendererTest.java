...
class CanonicalExcelTemplateRendererTest {
    ...
    @Test
    void serverRendererNormalizesStaleSampleFormattingInRepeatingRows() throws Exception {
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Invoice");
    ...
    @Test
    void serverRendererRemovesDuplicateHighlightFromMappedItemColumnsOnly() throws Exception {
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Invoice");
            Row template = sheet.createRow(0);
