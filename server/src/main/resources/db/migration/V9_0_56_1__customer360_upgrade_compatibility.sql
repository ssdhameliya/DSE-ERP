-- Same application release 9.0.56 corrective migration for databases that already applied V9_0_56.
-- Normalize Customer 360 relationship/attachment timestamp columns to the server's canonical text storage.
...
ALTER TABLE document_attachment ADD COLUMN IF NOT EXISTS created_by TEXT;
ALTER TABLE document_attachment ADD COLUMN IF NOT EXISTS created_at TEXT;
ALTER TABLE document_attachment ALTER COLUMN created_at TYPE TEXT USING created_at::text;
UPDATE document_attachment SET created_at=COALESCE(NULLIF(created_at,''),CURRENT_TIMESTAMP::text);
ALTER TABLE document_attachment ALTER COLUMN created_at SET DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE document_attachment ALTER COLUMN created_at SET NOT NULL;
