...
public class InsightsService {
 ...
 @Transactional(readOnly=true) public InsightDtos.ReportBundle report(String from,String to){
   ...
   List<InsightDtos.PointDto> cp=points("SELECT COALESCE(pm.name,'Unknown Customer'),SUM(sh.total_amount) amount FROM sales_header sh LEFT JOIN party_master pm ON pm.id=sh.customer_id WHERE UPPER(COALESCE(sh.document_status,'')) NOT IN ('DELETED','CANCELLED','REJECTED','PENDING APPROVAL') AND CAST(sh.invoice_date AS DATE) BETWEEN ? AND ? GROUP BY pm.id,pm.name ORDER BY amount DESC LIMIT 5",a);
   List<InsightDtos.PointDto> ip=points("SELECT COALESCE(im.description,sl.item_code),SUM(sl.line_total) amount FROM sales_line sl JOIN sales_header sh ON sh.id=sl.sales_id LEFT JOIN item_master im ON im.item_code=sl.item_code WHERE UPPER(COALESCE(sh.document_status,'')) NOT IN ('DELETED','CANCELLED','REJECTED','PENDING APPROVAL') AND CAST(sh.invoice_date AS DATE) BETWEEN ? AND ? GROUP BY sl.item_code,im.description ORDER BY amount DESC LIMIT 5",a);
   List<InsightDtos.ReportRow> sr=rows("SELECT sh.invoice_no,CAST(sh.invoice_date AS DATE),pm.name,sh.total_amount,COALESCE(sh.payment_status,'PENDING') FROM sales_header sh LEFT JOIN party_master pm ON pm.id=sh.customer_id WHERE UPPER(COALESCE(sh.document_status,'')) NOT IN ('DELETED','CANCELLED','REJECTED','PENDING APPROVAL') AND CAST(sh.invoice_date AS DATE) BETWEEN ? AND ? ORDER BY sh.invoice_date DESC,sh.id DESC LIMIT 8",a);
   List<InsightDtos.ReportRow> pr=rows("SELECT ph.invoice_no,CAST(ph.invoice_date AS DATE),pm.name,ph.total_amount,COALESCE(ph.payment_status,'PENDING') FROM purchase_header ph LEFT JOIN party_master pm ON pm.id=ph.supplier_id WHERE UPPER(COALESCE(ph.document_status,'')) NOT IN ('DELETED','CANCELLED','DRAFT','REJECTED','PENDING APPROVAL') AND CAST(ph.invoice_date AS DATE) BETWEEN ? AND ? ORDER BY ph.invoice_date DESC,ph.id DESC LIMIT 8",a);
