...
public class InsightsService {
 ...
 public InsightDtos.DashboardBundle dashboard(String period){
   ...
   double cash=dashboardSection("cash position",()->{double receipts=n("SELECT COALESCE(SUM(amount),0) FROM payment_record WHERE UPPER(document_type)='SALE'");double supplierPayments=n("SELECT COALESCE(SUM(amount),0) FROM payment_record WHERE UPPER(document_type)='PURCHASE'");double salesRefunds=n("SELECT COALESCE(SUM(rr.amount),0) FROM return_refund rr JOIN return_register r ON r.return_no=rr.return_no WHERE UPPER(COALESCE(r.return_type,'')) IN ('SALE RETURN','SALES RETURN')");double purchaseRefunds=n("SELECT COALESCE(SUM(rr.amount),0) FROM return_refund rr JOIN return_register r ON r.return_no=rr.return_no WHERE UPPER(COALESCE(r.return_type,''))='PURCHASE RETURN'");double bankDeposits=n("SELECT COALESCE(SUM(amount),0) FROM finance_register WHERE UPPER(COALESCE(voucher_type,''))='BANK DEPOSIT'");double bankWithdrawals=n("SELECT COALESCE(SUM(amount),0) FROM finance_register WHERE UPPER(COALESCE(voucher_type,''))='BANK WITHDRAWAL'");Double opening=jdbc.queryForObject("SELECT COALESCE(SUM(opening_balance),0) FROM (SELECT DISTINCT ON (COALESCE(account_number,'')) opening_balance FROM bank_statement_import WHERE opening_balance IS NOT NULL ORDER BY COALESCE(account_number,''),statement_from NULLS LAST,imported_at,id) x",Double.class);return (opening==null?0:opening)+receipts+purchaseRefunds+bankDeposits-supplierPayments-salesRefunds-bankWithdrawals-expense;},0d);
