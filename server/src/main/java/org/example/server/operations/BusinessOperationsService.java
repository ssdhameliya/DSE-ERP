...
public class BusinessOperationsService {
 ...
 @Transactional(readOnly=true) public OperationDtos.SalePage salesPage(int page,int size,String q,String invoice,String customer,String from,String to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,String documentStatus,Double minAmount,Double maxAmount){
  ...
  if(!blank(docFilter)&&!"ALL".equals(docFilter)){String stored="UPPER(COALESCE(h.document_status,''))";String special="('CANCELLED','DELETED','RETURNED','PARTIALLY RETURNED','DRAFT','PENDING APPROVAL','REJECTED')";if(Set.of("CANCELLED","DELETED","RETURNED","PARTIALLY RETURNED","DRAFT","PENDING APPROVAL","REJECTED").contains(docFilter))where.add(stored+"=?",docFilter);else if("COMPLETED".equals(docFilter))where.add(stored+" NOT IN "+special+" AND "+balance+"<=0.01");else if("IN PROGRESS".equals(docFilter))where.add(stored+" NOT IN "+special+" AND COALESCE(h.paid_amount,0)>0 AND "+balance+">0.01");else if("PENDING".equals(docFilter))where.add(stored+" NOT IN "+special+" AND COALESCE(h.paid_amount,0)<=0 AND "+balance+">0.01");}
 ...
 @Transactional public OperationDtos.SaleDto duplicateSale(int id){
  ...
  OperationDtos.SaleDto request=new OperationDtos.SaleDto(null,null,BusinessClock.today().toString(),d.customer(),d.subtotal(),d.discountAmount(),d.gstAmount(),d.totalAmount(),"Duplicated from "+source.getInvoiceNo(),null,false,BusinessClock.today().plusDays(30).toString(),0,"PENDING",false,d.invoiceType(),d.salesperson(),"DUPLICATE",d.notes(),d.deliveryAddress(),d.paymentTerms(),d.transporter(),d.referenceNo(),d.poDate(),d.billingAddress(),d.gstType(),d.doorDelivery(),d.vehicleNumber(),d.contactPerson(),d.transportNote(),null,d.gstin(),d.billingGstin(),d.deliveryGstin(),d.sameAsBilling(),d.transporterGstin(),d.chargeType(),d.chargeAmount(),d.contactPersonMobile(),"PENDING",null,d.quantity(),d.charges(),d.lines(),0);
 ...
 @Transactional(readOnly=true) public OperationDtos.PurchasePage purchasesPage(int page,int size,String q,String supplier,String from,String to,String paymentStatus,String mail,String documentStatus){
  int safeSize=Math.max(10,Math.min(size,200)),safePage=Math.max(0,page);SqlWhere where=new SqlWhere("UPPER(COALESCE(h.document_status,''))<>'DELETED'");String global=trim(q),supplierFilter=trim(supplier),fromDate=trim(from),toDate=trim(to),payment=up(paymentStatus),mailFilter=up(mail),docFilter=up(documentStatus);
  ...
  if(!blank(docFilter)&&!"ALL".equals(docFilter)){String stored="UPPER(COALESCE(h.document_status,''))";String special="('CANCELLED','DELETED','RETURNED','PARTIALLY RETURNED','DRAFT','PENDING APPROVAL','REJECTED')";if(Set.of("CANCELLED","DELETED","RETURNED","PARTIALLY RETURNED","DRAFT","PENDING APPROVAL","REJECTED").contains(docFilter))where.add(stored+"=?",docFilter);else if("COMPLETED".equals(docFilter))where.add(stored+" NOT IN "+special+" AND "+balance+"<=0.01");else if("IN PROGRESS".equals(docFilter))where.add(stored+" NOT IN "+special+" AND "+effectivePaid+">0 AND "+balance+">0.01");else if("PENDING".equals(docFilter))where.add(stored+" NOT IN "+special+" AND "+effectivePaid+"<=0 AND "+balance+">0.01");}
  long total=jdbc.queryForObject("SELECT COUNT(*)"+join+where.sql(),Long.class,where.args());int totalPages=total==0?0:(int)Math.ceil(total/(double)safeSize);if(totalPages>0&&safePage>=totalPages)safePage=totalPages-1;
 ...
 private void postSaleStock(int id){for(var l:salesLines.findBySalesIdOrderByIdAsc(id)){double cost=currentAverageCost(l.getItemCode(),n(l.getUnitCostSnapshot()));l.setUnitCostSnapshot(money(cost).doubleValue());salesLines.save(l);changeStockCost(l.getItemCode(),-n(l.getQuantity()),true,cost,"SALE",id);}}
 ...
 private void replacePurchaseLines(int id,List<OperationDtos.LineDto> ls,boolean skipStock){
  ...
  for(var d:ls){
   ...
   double unitCost=d.quantity()>0?money(calc.taxableAmount()/d.quantity()).doubleValue():0;
   ...
   l.setItemDescriptionSnapshot(item.getDescription());l.setHsnSnapshot(item.getHsn());l.setUnitSnapshot(item.getUnit());l.setItemRemarksSnapshot(item.getRemarks());l.setUnitCostSnapshot(unitCost);
...
@Transactional public void applyStockDelta(String code,double delta,boolean enforce){changeStock(code,delta,enforce);}
 ...
 private static double purchaseUnitCost(PurchaseLineEntity l){double q=n(l.getQuantity());if(q<=0)return 0;double gross=n(l.getRate())*q,discount=n(l.getDiscountAmount());return DocumentCalculationEngine.money(Math.max(0,gross-discount)/q);}

 private OperationDtos.SalesMetrics salesMetrics(SqlWhere where){
  String date=sqlDate("h.invoice_date"),due=sqlDate("h.due_date"),balance="GREATEST(COALESCE(h.total_amount,0)-COALESCE(h.paid_amount,0),0)";
  String join=" FROM sales_header h LEFT JOIN party_master p ON p.id=h.customer_id ";
  String active=where.sql()+" AND UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED')";
 ...
 private OperationDtos.PurchaseMetrics purchaseMetrics(SqlWhere where){
  ...
  String join=" FROM purchase_header h LEFT JOIN party_master p ON p.id=h.supplier_id LEFT JOIN (SELECT document_id,COALESCE(SUM(amount),0) recorded_paid FROM payment_record WHERE UPPER(document_type)='PURCHASE' GROUP BY document_id) pr ON pr.document_id=h.id ";
  String active=where.sql()+" AND UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED','DRAFT')";
  Object[] args=where.args();
  ...
  String lineJoin=" FROM purchase_line l JOIN purchase_header h ON h.id=l.purchase_id LEFT JOIN party_master p ON p.id=h.supplier_id LEFT JOIN (SELECT document_id,COALESCE(SUM(amount),0) recorded_paid FROM payment_record WHERE UPPER(document_type)='PURCHASE' GROUP BY document_id) pr ON pr.document_id=h.id ";
 ...
 private OperationDtos.LineDto line(SalesLineEntity l){String desc=blank(l.getItemDescriptionSnapshot())?items.findByItemCode(l.getItemCode()).map(ItemEntity::getDescription).orElse(null):l.getItemDescriptionSnapshot();return new OperationDtos.LineDto(l.getItemCode(),desc,l.getHsnSnapshot(),l.getUnitSnapshot(),l.getItemRemarksSnapshot(),n(l.getQuantity()),n(l.getRate()),n(l.getDiscountPercent()),n(l.getDiscountAmount()),n(l.getGstPercent()),n(l.getLineTotal()));}
 private OperationDtos.LineDto line(PurchaseLineEntity l){ItemEntity item=items.findByItemCode(l.getItemCode()).orElse(null);String desc=blank(l.getItemDescriptionSnapshot())?(item==null?null:item.getDescription()):l.getItemDescriptionSnapshot();String hsn=blank(l.getHsnSnapshot())?(item==null?null:item.getHsn()):l.getHsnSnapshot();String unit=blank(l.getUnitSnapshot())?(item==null?null:item.getUnit()):l.getUnitSnapshot();String remarks=blank(l.getItemRemarksSnapshot())?(item==null?null:item.getRemarks()):l.getItemRemarksSnapshot();return new OperationDtos.LineDto(l.getItemCode(),desc,hsn,unit,remarks,n(l.getQuantity()),n(l.getRate()),n(l.getDiscountPercent()),n(l.getDiscountAmount()),n(l.getGstPercent()),n(l.getLineTotal()));}
