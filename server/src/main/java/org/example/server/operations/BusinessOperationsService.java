...
@Transactional public void applyStockDelta(String code,double delta,boolean enforce){changeStock(code,delta,enforce);}
 ...
 private String currentPaymentStatusSql(String effectivePaid,String headerAlias){String pending="GREATEST(COALESCE(rs.approved_return,0)-COALESCE(rs.settled_return,0),0)";String base="CASE WHEN COALESCE("+headerAlias+".total_amount,0)>0 AND ("+effectivePaid+")+0.0001>=COALESCE("+headerAlias+".total_amount,0) THEN 'PAID' WHEN ("+effectivePaid+")>0.0001 THEN 'PARTIAL' ELSE COALESCE(NULLIF(UPPER("+headerAlias+".payment_status),''),'PENDING') END";return "CASE WHEN COALESCE(rs.pending_approval,0)>0 THEN 'RETURN APPROVAL PENDING' WHEN COALESCE(rs.approved_return,0)>0 THEN CASE WHEN "+pending+"<=0.01 THEN 'RETURN PAID' WHEN COALESCE(rs.settled_return,0)>0 THEN 'RETURN PARTIAL' ELSE 'RETURN PENDING' END ELSE "+base+" END";}
 private String currentBalanceSql(String baseBalance){String pending="GREATEST(COALESCE(rs.approved_return,0)-COALESCE(rs.settled_return,0),0)";return "CASE WHEN COALESCE(rs.approved_return,0)>0 THEN "+pending+" ELSE ("+baseBalance+") END";}
 private String currentDueDateSql(String baseDue){String pending="GREATEST(COALESCE(rs.approved_return,0)-COALESCE(rs.settled_return,0),0)";return "CASE WHEN COALESCE(rs.approved_return,0)>0 AND "+pending+">0.01 THEN rs.return_due ELSE ("+baseDue+") END";}
