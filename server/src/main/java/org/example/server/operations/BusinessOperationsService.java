...
public class BusinessOperationsService {
 ...
 @Transactional(readOnly=true) public OperationDtos.FinanceMetrics financeMetrics(){
  LocalDate today=BusinessClock.today();LocalDate monthStart=today.withDayOfMonth(1),monthEnd=monthStart.plusMonths(1),yearStart=LocalDate.of(today.getYear(),1,1),yearEnd=yearStart.plusYears(1);
  double[] v=jdbc.query("""
    SELECT
      COALESCE(SUM(amount) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='BANK DEPOSIT'),0),
      COALESCE(SUM(amount) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='BANK WITHDRAWAL'),0),
      COUNT(*) FILTER (WHERE UPPER(COALESCE(voucher_type,'')) IN ('BANK DEPOSIT','BANK WITHDRAWAL') AND dse_safe_date(voucher_date)>=? AND dse_safe_date(voucher_date)<?),
      COUNT(*) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='BANK DEPOSIT'),
      COUNT(*) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='BANK WITHDRAWAL'),
      COALESCE(SUM(amount) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='EXPENSE' AND dse_safe_date(voucher_date)>=? AND dse_safe_date(voucher_date)<?),0),
      COALESCE(SUM(amount) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='EXPENSE' AND dse_safe_date(voucher_date)>=? AND dse_safe_date(voucher_date)<?),0),
      COUNT(*) FILTER (WHERE UPPER(COALESCE(voucher_type,''))='EXPENSE' AND dse_safe_date(voucher_date)>=? AND dse_safe_date(voucher_date)<?)
    FROM finance_register
    """,(r,i)->new double[]{r.getDouble(1),r.getDouble(2),r.getDouble(3),r.getDouble(4),r.getDouble(5),r.getDouble(6),r.getDouble(7),r.getDouble(8)},monthStart,monthEnd,monthStart,monthEnd,yearStart,yearEnd,monthStart,monthEnd).getFirst();
  double[] pending=jdbc.query("SELECT COUNT(*),COALESCE(SUM(CASE WHEN credit_amount>0 THEN credit_amount ELSE debit_amount END),0) FROM bank_statement_transaction WHERE UPPER(COALESCE(status,'UNMATCHED')) IN ('UNMATCHED','SUGGESTED','REVIEW')",(r,i)->new double[]{r.getDouble(1),r.getDouble(2)}).getFirst();
  List<Object[]> top=jdbc.query("SELECT COALESCE(NULLIF(TRIM(category),''),'Other'),COALESCE(SUM(amount),0) total FROM finance_register WHERE UPPER(COALESCE(voucher_type,''))='EXPENSE' GROUP BY 1 ORDER BY total DESC LIMIT 1",(r,i)->new Object[]{r.getString(1),r.getDouble(2)});
  String topName=top.isEmpty()?"No expenses":String.valueOf(top.getFirst()[0]);double topValue=top.isEmpty()?0:((Number)top.getFirst()[1]).doubleValue();
  return new OperationDtos.FinanceMetrics(cashPosition.cashPosition(),v[0],v[1],(long)v[2],(long)v[3],(long)v[4],v[5],v[6],(long)v[7],topName,topValue,(long)pending[0],pending[1]);
 }
 ...
 private static boolean sameQuantity(Number a,double b){return BigDecimal.valueOf(n(a)).setScale(4,RoundingMode.HALF_UP).compareTo(BigDecimal.valueOf(b).setScale(4,RoundingMode.HALF_UP))==0;}
...
@Transactional public void applyStockDelta(String code,double delta,boolean enforce){changeStock(code,delta,enforce);}
 ...
 private OperationDtos.LineDto line(SalesLineEntity l){return new OperationDtos.LineDto(l.getItemCode(),l.getItemDescriptionSnapshot(),l.getCategorySnapshot(),l.getHsnSnapshot(),l.getUnitSnapshot(),l.getItemRemarksSnapshot(),n(l.getQuantity()),n(l.getRate()),n(l.getDiscountPercent()),n(l.getDiscountAmount()),n(l.getGstPercent()),n(l.getLineTotal()));}
 private OperationDtos.LineDto line(PurchaseLineEntity l){return new OperationDtos.LineDto(l.getItemCode(),l.getItemDescriptionSnapshot(),l.getCategorySnapshot(),l.getHsnSnapshot(),l.getUnitSnapshot(),l.getItemRemarksSnapshot(),n(l.getQuantity()),n(l.getRate()),n(l.getDiscountPercent()),n(l.getDiscountAmount()),n(l.getGstPercent()),n(l.getLineTotal()));}
...
private void copySale(OperationDtos.SaleDto d,SalesHeaderEntity h){h.setInvoiceNo(d.invoiceNo());h.setInvoiceDate(d.invoiceDate());h.setCustomer(requirePartyReference(d.customer()==null?null:d.customer().id(),"CUSTOMER","Customer",false));String taxType=blank(d.gstType())?"GST":d.gstType();DocumentCalculationEngine.Totals totals=documentTotals(d.lines(),normalizedCharges(d),taxType);h.setSubtotal(totals.itemTaxable());h.setDiscountAmount(totals.discountAmount());h.setGstAmount(totals.taxAmount());h.setTotalAmount(totals.grandTotal());h.setRemarks(d.remarks());h.setDueDate(d.dueDate());h.setWhatsappSent(d.whatsappSent()?1:0);h.setInvoiceType(d.invoiceType());h.setSalesperson(d.salesperson());h.setSource(d.source());h.setNotes(d.notes());h.setDeliveryAddress(d.deliveryAddress());h.setPaymentTerms(d.paymentTerms());h.setTransporter(d.transporter());h.setReferenceNo(d.referenceNo());h.setPoDate(d.poDate());h.setBillingAddress(d.billingAddress());h.setGstType(taxType);h.setDoorDelivery(d.doorDelivery());h.setVehicleNumber(d.vehicleNumber());h.setContactPerson(d.contactPerson());h.setTransportNote(d.transportNote());h.setOrderNo(customerPoOrderNo(d.orderNo()));h.setGstin(d.gstin());h.setBillingGstin(d.billingGstin());h.setDeliveryGstin(d.deliveryGstin());h.setSameAsBilling(d.sameAsBilling());h.setTransporterGstin(d.transporterGstin());List<OperationDtos.ChargeDto> charges=normalizedCharges(d);OperationDtos.ChargeDto first=charges.isEmpty()?null:charges.get(0);h.setChargeType(first==null?"":first.chargeType());h.setChargeAmount(first==null?0:first.amount());h.setContactPersonMobile(d.contactPersonMobile());h.setDocumentStatus(d.documentStatus());}
 ...
 private String configuredPreviewAtomicSql(String lookupCode,String fallback,String table,String column){
  return configuredReferenceSql(lookupCode,fallback,table,column,false);
 }
 private String configuredNextAtomicSql(String lookupCode,String fallback,String table,String column){
 ...
 private String configuredReferenceSql(String lookupCode,String fallback,String table,String column,boolean allocate){
  ...
  Long observedValue=jdbc.queryForObject("SELECT COALESCE(MAX(((regexp_match("+column+", ?))[1])::bigint),0)+1 FROM "+table+" WHERE "+column+" ~ ?",Long.class,regex,regex);long observed=observedValue==null?1L:Math.max(1L,observedValue);
  if(!allocate){List<Long> stored=jdbc.query("SELECT next_value FROM reference_counter WHERE counter_key=?",(r,i)->r.getLong(1),counterKey);long candidate=stored.isEmpty()?observed:Math.max(observed,stored.getFirst()+1L);return prefix+String.format(Locale.ROOT,"%0"+width+"d",candidate)+suffix;}
  Long allocatedValue=jdbc.queryForObject("INSERT INTO reference_counter(counter_key,next_value,updated_at) VALUES(?,?,?) ON CONFLICT(counter_key) DO UPDATE SET next_value=GREATEST(reference_counter.next_value+1,EXCLUDED.next_value),updated_at=EXCLUDED.updated_at RETURNING next_value",Long.class,counterKey,observed,BusinessClock.nowUtcText());long allocated=allocatedValue==null?observed:allocatedValue;return prefix+String.format(Locale.ROOT,"%0"+width+"d",allocated)+suffix;
 }
 private static String pgRegex(String value){
  if(value==null)return "";StringBuilder out=new StringBuilder(value.length()*2);String meta="\\.^$|?*+()[]{}";
  for(int i=0;i<value.length();i++){char c=value.charAt(i);if(meta.indexOf(c)>=0)out.append('\\');out.append(c);}return out.toString();
 }
