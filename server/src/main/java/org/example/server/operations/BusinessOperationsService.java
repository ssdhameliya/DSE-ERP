...
public class BusinessOperationsService {
 ...
 @Transactional public OperationDtos.SaleDto saveSale(OperationDtos.SaleDto d){SalesHeaderEntity h=new SalesHeaderEntity(); copySale(d,h); if(blank(h.getInvoiceNo())||sales.existsByInvoiceNo(h.getInvoiceNo()))h.setInvoiceNo(nextSalesInvoice()); h.setCreatedAt(BusinessClock.now().toString()); h.setEmailSent(0); h.setDocumentStatus(blank(h.getDocumentStatus())?"PENDING":h.getDocumentStatus()); h=sales.save(h); replaceSaleLines(h.getId(),d.lines(),false);replaceSaleCharges(h.getId(),normalizedCharges(d)); return saleDto(h,true);}
 ...
 @Transactional public void deleteSale(String invoice){SalesHeaderEntity h=sales.findByInvoiceNo(invoice).orElseThrow(()->new IllegalArgumentException("Sale not found: "+invoice));assertDocumentHasNoPayments("SALE",h.getId(),n(h.getPaidAmount()),h.getPaymentStatus(),"deleted");if(!"DELETED".equalsIgnoreCase(h.getDocumentStatus()))restoreSaleStock(h.getId());h.setDocumentStatus("DELETED");}
 @Transactional public void cancelSale(String invoice){SalesHeaderEntity h=sales.findByInvoiceNo(invoice).orElseThrow(()->new IllegalArgumentException("Sale not found: "+invoice));assertDocumentHasNoPayments("SALE",h.getId(),n(h.getPaidAmount()),h.getPaymentStatus(),"cancelled");if(!"CANCELLED".equalsIgnoreCase(h.getDocumentStatus())&&!"DELETED".equalsIgnoreCase(h.getDocumentStatus()))restoreSaleStock(h.getId());h.setDocumentStatus("CANCELLED");}
 ...
 @Transactional public OperationDtos.PurchaseDto savePurchase(OperationDtos.PurchaseDto d){PurchaseHeaderEntity h=new PurchaseHeaderEntity();copyPurchase(d,h);if(blank(h.getInvoiceNo())||purchases.existsByInvoiceNo(h.getInvoiceNo()))h.setInvoiceNo(nextPurchaseInvoice());h.setCreatedAt(BusinessClock.now().toString());h=purchases.save(h);replacePurchaseLines(h.getId(),d.lines(),false);return purchaseDto(h,true);}
 ...
 @Transactional public OperationDtos.FinanceDto saveFinance(OperationDtos.FinanceDto d){FinanceRegisterEntity e=new FinanceRegisterEntity();copyFinance(d,e);if(blank(e.getVoucherNo()))e.setVoucherNo(nextVoucher());e.setCreatedAt(BusinessClock.now().toString());return financeDto(finance.save(e));}
