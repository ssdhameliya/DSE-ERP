...
public class BusinessOperationsService {
 ...
 @Transactional(readOnly=true) public OperationDtos.SalePage salesPage(int page,int size,String q,String invoice,String customer,String from,String to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,Double minAmount,Double maxAmount){
  ...
  String global=trim(q),invoiceFilter=trim(invoice),customerFilter=trim(customer),fromDate=trim(from),toDate=trim(to),payment=up(paymentStatus),dueFilter=up(due),mailFilter=up(mail),whatsappFilter=up(whatsapp),typeFilter=up(invoiceType);
  ...
  String invoiceDate=sqlDate("h.invoice_date"),dueDate=sqlDate("h.due_date"),balance="GREATEST(COALESCE(h.total_amount,0)-COALESCE(h.paid_amount,0),0)";
  ...
  if(!blank(payment)&&!"ALL".equals(payment)){if("OVERDUE".equals(payment))where.add(balance+">0.01 AND "+dueDate+"<CURRENT_DATE");else where.add("UPPER(COALESCE(h.payment_status,'PENDING'))=?",payment);}
  ...
  String join=" FROM sales_header h LEFT JOIN party_master p ON p.id=h.customer_id ";
  ...
  String active=where.sql()+" AND UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED')";var totalsRow=jdbc.query("SELECT COUNT(*),COALESCE(SUM(h.total_amount),0),COALESCE(SUM(h.paid_amount),0),COALESCE(SUM("+balance+"),0)"+join+active,(r,i)->new OperationDtos.RegisterTotals(r.getLong(1),r.getDouble(2),r.getDouble(3),r.getDouble(4)),where.args()).getFirst();
  return new OperationDtos.SalePage(rows,safePage,safeSize,total,totalPages,totalsRow,salesMetrics(),saleCustomerOptions());
 }
 ...
 @Transactional(readOnly=true) public OperationDtos.PurchasePage purchasesPage(int page,int size,String q,String supplier,String from,String to,String paymentStatus,String mail){
  int safeSize=Math.max(10,Math.min(size,200)),safePage=Math.max(0,page);SqlWhere where=new SqlWhere("UPPER(COALESCE(h.document_status,''))<>'DELETED'");String global=trim(q),supplierFilter=trim(supplier),fromDate=trim(from),toDate=trim(to),payment=up(paymentStatus),mailFilter=up(mail);
  String join=" FROM purchase_header h LEFT JOIN party_master p ON p.id=h.supplier_id LEFT JOIN (SELECT document_id,COALESCE(SUM(amount),0) recorded_paid FROM payment_record WHERE UPPER(document_type)='PURCHASE' GROUP BY document_id) pr ON pr.document_id=h.id ";
  String invoiceDate=sqlDate("h.invoice_date"),dueDate=sqlDate("h.due_date");String effectivePaid="LEAST(GREATEST(COALESCE(h.total_amount,0),0),GREATEST(COALESCE(h.paid_amount,0),COALESCE(pr.recorded_paid,0),CASE WHEN UPPER(COALESCE(h.payment_status,'')) IN ('PAID','SETTLED') THEN COALESCE(h.total_amount,0) ELSE 0 END))";String balance="GREATEST(COALESCE(h.total_amount,0)-("+effectivePaid+"),0)";String derivedStatus="CASE WHEN COALESCE(h.total_amount,0)>0 AND ("+effectivePaid+")+0.0001>=COALESCE(h.total_amount,0) THEN 'PAID' WHEN ("+effectivePaid+")>0.0001 THEN 'PARTIAL' ELSE COALESCE(NULLIF(UPPER(h.payment_status),''),'PENDING') END";
  ...
  if(!blank(fromDate))where.add(invoiceDate+">=TO_DATE(?,'YYYY-MM-DD')",fromDate);if(!blank(toDate))where.add(invoiceDate+"<=TO_DATE(?,'YYYY-MM-DD')",toDate);
  if(!blank(payment)&&!"ALL".equals(payment)){if("OVERDUE".equals(payment))where.add(balance+">0.01 AND "+dueDate+"<CURRENT_DATE");else where.add("("+derivedStatus+")=?",payment);}if("SENT".equals(mailFilter))where.add("COALESCE(h.email_sent,0)<>0");else if("NOT SENT".equals(mailFilter))where.add("COALESCE(h.email_sent,0)=0");
  long total=jdbc.queryForObject("SELECT COUNT(*)"+join+where.sql(),Long.class,where.args());int totalPages=total==0?0:(int)Math.ceil(total/(double)safeSize);if(totalPages>0&&safePage>=totalPages)safePage=totalPages-1;
  ...
  String active=where.sql()+" AND UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED','DRAFT')";var filteredTotals=jdbc.query("SELECT COUNT(*),COALESCE(SUM(h.total_amount),0),COALESCE(SUM("+effectivePaid+"),0),COALESCE(SUM("+balance+"),0)"+join+active,(r,i)->new OperationDtos.RegisterTotals(r.getLong(1),r.getDouble(2),r.getDouble(3),r.getDouble(4)),where.args()).getFirst();return new OperationDtos.PurchasePage(rows,safePage,safeSize,total,totalPages,filteredTotals,purchaseMetrics(),purchaseSupplierOptions());
 }
 ...
 @Transactional public OperationDtos.FinanceDto saveFinance(OperationDtos.FinanceDto d){CurrentUser.requirePermission("BANK_EXPENSE.CREATE","Create finance entry");FinanceRegisterEntity e=new FinanceRegisterEntity();copyFinance(d,e);if(blank(e.getVoucherNo()))e.setVoucherNo(nextVoucher());e.setCreatedAt(BusinessClock.nowUtcText());e=finance.saveAndFlush(e);audit.log("FINANCE",e.getId(),"CREATED",e.getVoucherNo());return financeDto(e);}
 @Transactional public OperationDtos.FinanceDto updateFinance(OperationDtos.FinanceDto d){CurrentUser.requirePermission("BANK_EXPENSE.EDIT","Edit finance entry");FinanceRegisterEntity e=finance.findById(req(d.id())).orElseThrow(()->new IllegalArgumentException("Finance entry not found"));assertVersion(d.rowVersion(),e.getRowVersion(),"Finance entry "+e.getVoucherNo());copyFinance(d,e);finance.flush();audit.log("FINANCE",e.getId(),"UPDATED",e.getVoucherNo());return financeDto(e);}
 @Transactional public void deleteFinance(int id,long rowVersion){CurrentUser.requirePermission("BANK_EXPENSE.DELETE","Delete finance entry");FinanceRegisterEntity e=finance.findById(id).orElseThrow(()->new IllegalArgumentException("Finance entry not found"));assertVersion(rowVersion,e.getRowVersion(),"Finance entry "+e.getVoucherNo());if(n(e.getReconciled())!=0)throw new IllegalStateException("Reconciled finance entries must be reversed from Bank Statement before deletion.");String ref=e.getVoucherNo();finance.delete(e);audit.log("FINANCE",id,"DELETED",ref);}
...
private OperationDtos.SaleDto saleSummaryDto(SalesHeaderEntity h,double qty,List<OperationDtos.ChargeDto> storedCharges){var p=h.getCustomer();List<OperationDtos.ChargeDto> charges=storedCharges;if(charges.isEmpty()&&n(h.getChargeAmount())>0)charges=List.of(new OperationDtos.ChargeDto(blank(h.getChargeType())?"Charges":h.getChargeType(),n(h.getChargeAmount()),false,0));return new OperationDtos.SaleDto(h.getId(),h.getInvoiceNo(),dateOnly(h.getInvoiceDate()),party(p),n(h.getSubtotal()),n(h.getDiscountAmount()),n(h.getGstAmount()),n(h.getTotalAmount()),h.getRemarks(),h.getCreatedAt(),n(h.getEmailSent())!=0,dateOnly(h.getDueDate()),n(h.getPaidAmount()),h.getPaymentStatus(),n(h.getWhatsappSent())!=0,h.getInvoiceType(),h.getSalesperson(),h.getSource(),h.getNotes(),h.getDeliveryAddress(),h.getPaymentTerms(),h.getTransporter(),h.getReferenceNo(),dateOnly(h.getPoDate()),h.getBillingAddress(),h.getGstType(),h.getDoorDelivery(),h.getVehicleNumber(),h.getContactPerson(),h.getTransportNote(),customerPoOrderNo(h.getOrderNo()),h.getGstin(),h.getBillingGstin(),h.getDeliveryGstin(),!Boolean.FALSE.equals(h.getSameAsBilling()),h.getTransporterGstin(),h.getChargeType(),n(h.getChargeAmount()),h.getContactPersonMobile(),h.getDocumentStatus(),h.getAttachmentPath(),qty,charges,List.of(),nv(h.getRowVersion()));}
 ...
 private Map<Integer,Double> recordedPurchasePayments(Collection<Integer> ids){Map<Integer,Double> out=new HashMap<>();String in=idCsv(ids);if(in.isBlank())return out;jdbc.query("SELECT document_id,COALESCE(SUM(amount),0) FROM payment_record WHERE UPPER(document_type)='PURCHASE' AND document_id IN ("+in+") GROUP BY document_id",r->out.put(r.getInt(1),r.getDouble(2)));return out;}
 private Map<Integer,Double> saleQuantityTotals(Collection<Integer> ids){Map<Integer,Double> out=new HashMap<>();String in=idCsv(ids);if(in.isBlank())return out;jdbc.query("SELECT sales_id,COALESCE(SUM(quantity),0) FROM sales_line WHERE sales_id IN ("+in+") GROUP BY sales_id",r->out.put(r.getInt(1),r.getDouble(2)));return out;}
 private Map<Integer,List<OperationDtos.ChargeDto>> saleChargeSummaries(Collection<Integer> ids){Map<Integer,List<OperationDtos.ChargeDto>> out=new HashMap<>();String in=idCsv(ids);if(in.isBlank())return out;jdbc.query("SELECT sales_id,charge_name,COALESCE(amount,0),CASE WHEN COALESCE(taxable,false) THEN 1 ELSE 0 END,COALESCE(gst_percent,0) FROM sales_charge WHERE sales_id IN ("+in+") ORDER BY sales_id,sequence_no,id",r->out.computeIfAbsent(r.getInt(1),k->new ArrayList<>()).add(new OperationDtos.ChargeDto(r.getString(2),r.getDouble(3),r.getInt(4)!=0,r.getDouble(5))));return out;}
 private Map<Integer,Double> purchaseQuantityTotals(Collection<Integer> ids){Map<Integer,Double> out=new HashMap<>();String in=idCsv(ids);if(in.isBlank())return out;jdbc.query("SELECT purchase_id,COALESCE(SUM(quantity),0) FROM purchase_line WHERE purchase_id IN ("+in+") GROUP BY purchase_id",r->out.put(r.getInt(1),r.getDouble(2)));return out;}
 private Map<Integer,List<OperationDtos.ChargeDto>> purchaseChargeSummaries(Collection<Integer> ids){Map<Integer,List<OperationDtos.ChargeDto>> out=new HashMap<>();String in=idCsv(ids);if(in.isBlank())return out;jdbc.query("SELECT purchase_id,charge_name,amount,taxable,gst_percent FROM purchase_charge WHERE purchase_id IN ("+in+") ORDER BY purchase_id,sequence_no,id",r->out.computeIfAbsent(r.getInt(1),k->new ArrayList<>()).add(new OperationDtos.ChargeDto(r.getString(2),r.getDouble(3),r.getBoolean(4),r.getDouble(5))));return out;}
 private OperationDtos.SalesMetrics salesMetrics(){String date=sqlDate("h.invoice_date"),due=sqlDate("h.due_date"),balance="GREATEST(COALESCE(h.total_amount,0)-COALESCE(h.paid_amount,0),0)";String active=" FROM sales_header h WHERE UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED')";var base=jdbc.query("SELECT COALESCE(SUM(h.total_amount),0),COUNT(*),COALESCE(SUM(CASE WHEN "+date+"=CURRENT_DATE THEN h.total_amount ELSE 0 END),0),SUM(CASE WHEN "+date+"=CURRENT_DATE THEN 1 ELSE 0 END),COALESCE(SUM("+balance+"),0),SUM(CASE WHEN "+balance+">0.01 THEN 1 ELSE 0 END),COALESCE(SUM(CASE WHEN "+balance+">0.01 AND "+due+"<CURRENT_DATE THEN "+balance+" ELSE 0 END),0),SUM(CASE WHEN "+balance+">0.01 AND "+due+"<CURRENT_DATE THEN 1 ELSE 0 END),COALESCE(SUM(CASE WHEN "+balance+">0.01 AND "+due+" BETWEEN CURRENT_DATE AND CURRENT_DATE+7 THEN "+balance+" ELSE 0 END),0),SUM(CASE WHEN "+balance+">0.01 AND "+due+" BETWEEN CURRENT_DATE AND CURRENT_DATE+7 THEN 1 ELSE 0 END),SUM(CASE WHEN COALESCE(h.email_sent,0)<>0 THEN 1 ELSE 0 END)"+active,(r,i)->new double[]{r.getDouble(1),r.getLong(2),r.getDouble(3),r.getLong(4),r.getDouble(5),r.getLong(6),r.getDouble(7),r.getLong(8),r.getDouble(9),r.getLong(10),r.getLong(11)}).getFirst();double rate=base[1]<=0?0:base[10]*100d/base[1];List<OperationDtos.MetricPoint> dueBuckets=jdbc.query("SELECT bucket,amount FROM (SELECT 1 ord,'Due Today' bucket,COALESCE(SUM(CASE WHEN "+balance+">0.01 AND "+due+"<=CURRENT_DATE THEN "+balance+" ELSE 0 END),0) amount"+active+" UNION ALL SELECT 2,'1-7 Days',COALESCE(SUM(CASE WHEN "+balance+">0.01 AND "+due+">CURRENT_DATE AND "+due+"<=CURRENT_DATE+7 THEN "+balance+" ELSE 0 END),0)"+active+" UNION ALL SELECT 3,'8-30 Days',COALESCE(SUM(CASE WHEN "+balance+">0.01 AND "+due+">CURRENT_DATE+7 AND "+due+"<=CURRENT_DATE+30 THEN "+balance+" ELSE 0 END),0)"+active+" UNION ALL SELECT 4,'Over 30 Days',COALESCE(SUM(CASE WHEN "+balance+">0.01 AND "+due+">CURRENT_DATE+30 THEN "+balance+" ELSE 0 END),0)"+active+") x ORDER BY ord",(r,i)->new OperationDtos.MetricPoint(r.getString(1),r.getDouble(2)));List<OperationDtos.MetricPoint> top=jdbc.query("SELECT COALESCE(p.name,'Unknown Customer'),COALESCE(SUM(h.total_amount),0) FROM sales_header h LEFT JOIN party_master p ON p.id=h.customer_id WHERE UPPER(COALESCE(h.document_status,''))<>'DELETED' GROUP BY COALESCE(p.name,'Unknown Customer') ORDER BY SUM(h.total_amount) DESC LIMIT 5",(r,i)->new OperationDtos.MetricPoint(r.getString(1),r.getDouble(2)));List<OperationDtos.MetricPoint> months=jdbc.query("SELECT LEFT(h.invoice_date,7),COALESCE(SUM(h.total_amount),0) FROM sales_header h WHERE UPPER(COALESCE(h.document_status,''))<>'DELETED' AND COALESCE(h.invoice_date,'') ~ '^\\d{4}-\\d{2}-\\d{2}' GROUP BY LEFT(h.invoice_date,7) ORDER BY LEFT(h.invoice_date,7) DESC LIMIT 7",(r,i)->new OperationDtos.MetricPoint(r.getString(1),r.getDouble(2)));Collections.reverse(months);return new OperationDtos.SalesMetrics(base[0],(long)base[1],base[2],(long)base[3],base[4],(long)base[5],base[6],(long)base[7],base[8],(long)base[9],rate,dueBuckets,top,months);}
 private OperationDtos.PurchaseMetrics purchaseMetrics(){String join=" FROM purchase_header h LEFT JOIN (SELECT document_id,COALESCE(SUM(amount),0) recorded_paid FROM payment_record WHERE UPPER(document_type)='PURCHASE' GROUP BY document_id) pr ON pr.document_id=h.id ";String effective="LEAST(GREATEST(COALESCE(h.total_amount,0),0),GREATEST(COALESCE(h.paid_amount,0),COALESCE(pr.recorded_paid,0),CASE WHEN UPPER(COALESCE(h.payment_status,'')) IN ('PAID','SETTLED') THEN COALESCE(h.total_amount,0) ELSE 0 END))";String active=" WHERE UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED','DRAFT')";var row=jdbc.query("SELECT COALESCE(SUM(h.total_amount),0),COUNT(*),COUNT(DISTINCT h.supplier_id),COALESCE(SUM("+effective+"),0)"+join+active,(r,i)->new double[]{r.getDouble(1),r.getLong(2),r.getLong(3),r.getDouble(4)}).getFirst();Double itemsQty=jdbc.queryForObject("SELECT COALESCE(SUM(l.quantity),0) FROM purchase_line l JOIN purchase_header h ON h.id=l.purchase_id WHERE UPPER(COALESCE(h.document_status,'')) NOT IN ('CANCELLED','DELETED','DRAFT')",Double.class);return new OperationDtos.PurchaseMetrics(row[0],(long)row[1],(long)row[2],n(itemsQty),row[3]);}
 private List<String> saleCustomerOptions(){return jdbc.query("SELECT DISTINCT p.name FROM sales_header h JOIN party_master p ON p.id=h.customer_id WHERE UPPER(COALESCE(h.document_status,''))<>'DELETED' AND COALESCE(p.name,'')<>'' ORDER BY p.name",(r,i)->r.getString(1));}
 private List<String> purchaseSupplierOptions(){return jdbc.query("SELECT DISTINCT p.name FROM purchase_header h JOIN party_master p ON p.id=h.supplier_id WHERE UPPER(COALESCE(h.document_status,''))<>'DELETED' AND COALESCE(p.name,'')<>'' ORDER BY p.name",(r,i)->r.getString(1));}
...
private void copySale(OperationDtos.SaleDto d,SalesHeaderEntity h){h.setInvoiceNo(d.invoiceNo());h.setInvoiceDate(d.invoiceDate());h.setCustomer(parties.findById(req(d.customer()==null?null:d.customer().id())).orElseThrow(()->new IllegalArgumentException("Customer not found")));DocumentCalculationEngine.Totals totals=documentTotals(d.lines(),normalizedCharges(d),d.gstType());h.setSubtotal(totals.itemTaxable());h.setDiscountAmount(totals.discountAmount());h.setGstAmount(totals.taxAmount());h.setTotalAmount(totals.grandTotal());h.setRemarks(d.remarks());h.setDueDate(d.dueDate());h.setPaidAmount(d.paidAmount());h.setPaymentStatus(d.paymentStatus());h.setWhatsappSent(d.whatsappSent()?1:0);h.setInvoiceType(d.invoiceType());h.setSalesperson(d.salesperson());h.setSource(d.source());h.setNotes(d.notes());h.setDeliveryAddress(d.deliveryAddress());h.setPaymentTerms(d.paymentTerms());h.setTransporter(d.transporter());h.setReferenceNo(d.referenceNo());h.setPoDate(d.poDate());h.setBillingAddress(d.billingAddress());h.setGstType(d.gstType());h.setDoorDelivery(d.doorDelivery());h.setVehicleNumber(d.vehicleNumber());h.setContactPerson(d.contactPerson());h.setTransportNote(d.transportNote());h.setOrderNo(customerPoOrderNo(d.orderNo()));h.setGstin(d.gstin());h.setBillingGstin(d.billingGstin());h.setDeliveryGstin(d.deliveryGstin());h.setSameAsBilling(d.sameAsBilling());h.setTransporterGstin(d.transporterGstin());List<OperationDtos.ChargeDto> charges=normalizedCharges(d);OperationDtos.ChargeDto first=charges.isEmpty()?null:charges.get(0);h.setChargeType(first==null?"":first.chargeType());h.setChargeAmount(first==null?0:first.amount());h.setContactPersonMobile(d.contactPersonMobile());h.setDocumentStatus(d.documentStatus());}
 ...
 @Transactional(propagation=Propagation.REQUIRES_NEW) public String nextConfiguredReference(String lookupCode,String fallback,java.util.function.Supplier<List<String>> existingSupplier){return configuredNextAtomic(lookupCode,fallback,existingSupplier);}
 private String configuredNextAtomic(String lookupCode,String fallback,java.util.function.Supplier<List<String>> existingSupplier){
  ...
  List<Long> current=jdbc.query("SELECT next_value FROM reference_counter WHERE counter_key=? FOR UPDATE",(r,i)->r.getLong(1),counterKey);
  if(!current.isEmpty()){long allocated=jdbc.queryForObject("UPDATE reference_counter SET next_value=next_value+1,updated_at=? WHERE counter_key=? RETURNING next_value",Long.class,BusinessClock.nowUtcText(),counterKey);return prefix+String.format(Locale.ROOT,"%0"+width+"d",allocated)+suffix;}
  long observed=1L;List<String> existing=existingSupplier==null?List.of():existingSupplier.get();
  for(String value:existing==null?List.<String>of():existing){if(value==null||!value.startsWith(prefix)||!value.endsWith(suffix))continue;try{observed=Math.max(observed,Long.parseLong(value.substring(prefix.length(),value.length()-suffix.length()))+1L);}catch(Exception ignored){}}
 ...
 private static String sqlDate(String column){return "CASE WHEN COALESCE("+column+",'') ~ '^\\d{4}-\\d{2}-\\d{2}' THEN TO_DATE(LEFT("+column+",10),'YYYY-MM-DD') WHEN COALESCE("+column+",'') ~ '^\\d{2}/\\d{2}/\\d{4}' THEN TO_DATE(LEFT("+column+",10),'DD/MM/YYYY') WHEN COALESCE("+column+",'') ~ '^\\d{2}-\\d{2}-\\d{4}' THEN TO_DATE(LEFT("+column+",10),'DD-MM-YYYY') ELSE NULL END";}
