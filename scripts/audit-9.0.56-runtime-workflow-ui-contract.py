...
for p in ['pom.xml','server/pom.xml','desktop/pom.xml','shared/pom.xml']:
    need('9.0.56' in text(p),f'9.0.56 missing from {p}')
for p in ['runtime/runtime-manifest.properties','desktop/src/main/resources/app-version.properties','server/src/main/resources/application.properties','shared/src/main/java/org/example/shared/RuntimeContract.java']:
    need('9.0.56' in text(p),f'9.0.56 missing from {p}')
need('9.0.56 - DEVELOPMENT / INTELLIJ ONLY' in text('Run DSE ERP.bat'),'launcher identity not 9.0.56')
# No markdown files
...
# 9.0.56 stabilization additions
...
# Same-version corrective runtime checks (9.0.56): no stale :8080 fallback, upgraded C360 schema safety,
...
# Runtime regressions confirmed from 9.0.56 field logs.
...
# Final same-version 9.0.56 consolidated corrective checks.
...
need('DSE ERP 9.0.56 - PRODUCTION WINDOWS BUILD' in text('Build Production Windows.bat'),'production build banner identity is not 9.0.56')
need('DSE ERP 9.0.56 - RELEASE GATE VERIFICATION' in text('Run Release Gates.bat'),'release-gate banner identity is not 9.0.56')
need('audit-release-gates-9.0.56.py' in text('.github/workflows/ci.yml') and 'audit-release-gates-9.0.56.py' in text('.github/workflows/release.yml'),'CI/release are not wired to the current 9.0.56 aggregate gate')

print(f'RUNTIME_WORKFLOW_UI_9_0_56_OK fxml={len(fxmls)} css=2 markdown=0 menu=DataImport->ProjectExecution numbering=save-only registration=qr')
