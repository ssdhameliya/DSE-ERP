...
req('9.0.32 runtime identity', 'APP_VERSION = "9.0.32"' in runtime and 'BUILD_REVISION = "9.0.32"' in runtime)
...
req('Bank history FXML no longer uses narrow 9.0.30 clamp', 'prefWidth="620"' not in bankf and 'minWidth="520"' not in bankf and 'maxWidth="760"' not in bankf and 'prefWidth="900"' in bankf and 'maxWidth="Infinity"' in bankf)
req('Bank history controller no longer snaps drawer to 520-680 px', 'Math.max(520' not in bank and 'Math.min(680' not in bank and 'historyShare=width>=1800?.52:width>=1450?.56:.60;' in bank)
req('Bank history width has one controller authority with post-layout reapply', 'setDividerPositions(divider)' in bank and 'Platform.runLater' in bank)
...
print('PASS: DSE ERP 9.0.32 payment/due separation + Bank Statement history root-cause contract')
