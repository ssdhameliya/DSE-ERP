...
req('<artifactId>dse-erp-parent</artifactId>\n  <version>9.0.37</version>' in root_pom and '<dse.phase>9.0.37</dse.phase>' in root_pom,
...
for name, pom in [('shared', shared_pom), ('server', server_pom), ('desktop', desktop_pom)]:
    req('<artifactId>dse-erp-parent</artifactId>' in pom and '<version>9.0.37</version>' in pom,
        f'{name} Maven parent version must be 9.0.18')
req('version=9.0.37' in app_version, 'desktop app-version.properties must be 9.0.18')
req('DEFAULT_VERSION="9.0.37"' in update_service, 'update fallback version must be 9.0.18')
req('runtime.phase=9.0.37' in runtime_manifest, 'runtime identity manifest phase must be 9.0.18')
