#!/usr/bin/env python3
"""DSE ERP 9.0.52 final IntelliJ/reporting/scheduler contract."""
...
need('<version>9.0.52</version>' in text('pom.xml') and '<dse.phase>9.0.52</dse.phase>' in text('pom.xml'),'root Maven identity is not 9.0.52')
need('APP_VERSION = "9.0.52"' in text('shared/src/main/java/org/example/shared/RuntimeContract.java'),'desktop/shared runtime identity is not 9.0.52')
need('dse.app.version=9.0.52' in text('server/src/main/resources/application.properties'),'server runtime identity is not 9.0.52')
need('runtime.phase=9.0.52' in text('runtime/runtime-manifest.properties'),'bundled runtime manifest is not 9.0.52')
...
need('MIN_COMFORTABLE_CARD = 170.0' in kpi,'responsive KPI comfortable width floor is not the 9.0.52 compact 170px value')
...
need('CREATE TABLE IF NOT EXISTS lookup_code_alias' in master_migration and "~ '^GEN[0-9]+$'" in master_migration,'9.0.52 must migrate historical GENxxx codes and retain aliases')
need("VALUES('auth.selfRegistrationRole', 'SALES')" in master_migration,'9.0.52 must seed an upgrade-safe configurable registration-role setting')
...
# Current release gates must be wired into CI/release. Historical feature audits may remain
# available for forensic/regression use without every historical filename being a live workflow step.
workflow_text=text('.github/workflows/ci.yml')+'\n'+text('.github/workflows/release.yml')
for required_audit in ('audit-release-gates-9.0.52.py','audit-9.0.52-ui-stabilization-contract.py'):
    need(required_audit in workflow_text,'current 9.0.52 release gate is not wired to CI/release: '+required_audit)
