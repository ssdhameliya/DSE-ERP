#!/usr/bin/env python3
"""DSE ERP 9.0.42 Phase 7 production-source cleanliness contract."""
...
JUNK_FILE_PATTERNS = (
    ...
    re.compile(r".*\.(?:class|log|tmp|bak|orig)$", re.I), re.compile(r".*~$"),
)
TEXT_EXTENSIONS = {
...
version_contract = {
    "desktop/src/main/resources/app-version.properties": "version=9.0.42",
    "runtime/runtime-manifest.properties": "runtime.phase=9.0.42",
    "pom.xml": "<version>9.0.42</version>",
    "desktop/pom.xml": "<version>9.0.42</version>",
    "server/pom.xml": "<version>9.0.42</version>",
}
for file_name, token in version_contract.items():
    ...
    if not path.exists() or token not in path.read_text(encoding="utf-8", errors="ignore"):
        fail(f"9.0.42 version contract missing from {file_name}: {token}")

# Temporary Phase-2 merge metadata must be gone after final canonicalization.
if (ROOT / "scripts/phase2-two-theme-source-map-9.0.42.json").exists():
