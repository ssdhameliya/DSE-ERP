...
need('<version>9.0.52</version>' in t('pom.xml') and '<dse.phase>9.0.52</dse.phase>' in t('pom.xml'),'root version')
need('APP_VERSION = "9.0.52"' in t('shared/src/main/java/org/example/shared/RuntimeContract.java'),'runtime version')
need('version=9.0.52' in t('desktop/src/main/resources/app-version.properties'),'desktop version')
need('dse.app.version=9.0.52' in t('server/src/main/resources/application.properties'),'server version')
...
need(len(list((R/'desktop/src/main/resources/fxml').rglob('*.fxml')))==63,'expected 63 FXML after Customer 360')
need(sorted(p.name for p in (R/'desktop/src/main/resources/css').glob('*.css'))==['dark-theme.css','light-theme.css'],'exactly two themes')
print('CUSTOMER_360_9_0_52_OK fxml=63 tabs=9 actions=Back/Edit/NewSale persistence=server-owned')
