...
req('<version>9.0.46</version>' in root_pom and '<dse.phase>9.0.46</dse.phase>' in root_pom,
    'root Maven release identity must be 9.0.18')
for name,pom in [('server',server_pom),('desktop',desktop_pom),('shared',shared_pom)]:
    req('<version>9.0.46</version>' in pom, f'{name} parent version must be 9.0.18')
...
req('version=9.0.46' in app_version and 'DEFAULT_VERSION="9.0.46"' in update,
    'desktop resource/updater identity must be 9.0.18')
if runtime_manifest:
    req('runtime.phase=9.0.46' in runtime_manifest, 'bundled runtime phase must be 9.0.18')
...
req('fx:id="lblCurrentVersion" text="9.0.46"' in safe_rollback,
