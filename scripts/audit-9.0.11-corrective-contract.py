...
req('<version>9.0.38</version>' in root_pom and '<dse.phase>9.0.38</dse.phase>' in root_pom,
    'root Maven release identity must be 9.0.18')
for name,pom in [('server',server_pom),('desktop',desktop_pom),('shared',shared_pom)]:
    req('<version>9.0.38</version>' in pom, f'{name} parent version must be 9.0.18')
req('APP_VERSION = "9.0.38"' in runtime and 'BUILD_REVISION = "9.0.38"' in runtime,
    'shared runtime identity must be 9.0.18')
req('dse.app.version=9.0.38' in props and 'dse.build.revision=9.0.38' in props,
    'server runtime identity must be 9.0.18')
req('version=9.0.38' in app_version and 'DEFAULT_VERSION="9.0.38"' in update,
    'desktop resource/updater identity must be 9.0.18')
if runtime_manifest:
    req('runtime.phase=9.0.38' in runtime_manifest, 'bundled runtime phase must be 9.0.18')
req('DSE ERP 9.0.38 - DEVELOPMENT / INTELLIJ ONLY' in run_bat,
    'IntelliJ launcher banner must be 9.0.18')
req('DSE ERP 9.0.38 - PRODUCTION WINDOWS BUILD' in build_bat,
    'production build banner must be 9.0.18')
req('DSE ERP 9.0.38 uses application-managed PostgreSQL.' in postgres_bat,
    'PostgreSQL launcher banner must be 9.0.18')
req('fx:id="lblCurrentVersion" text="9.0.38"' in safe_rollback,
