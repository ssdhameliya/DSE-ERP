#!/usr/bin/env python3
"""DSE ERP 9.0.49 final IntelliJ/reporting/scheduler contract."""
...
need('<version>9.0.52</version>' in text('pom.xml') and '<dse.phase>9.0.49</dse.phase>' in text('pom.xml'),'root Maven identity is not 9.0.49')
need('APP_VERSION = "9.0.52"' in text('shared/src/main/java/org/example/shared/RuntimeContract.java'),'desktop/shared runtime identity is not 9.0.49')
need('dse.app.version=9.0.52' in text('server/src/main/resources/application.properties'),'server runtime identity is not 9.0.49')
need('runtime.phase=9.0.52' in text('runtime/runtime-manifest.properties'),'bundled runtime manifest is not 9.0.49')
...
need('MIN_COMFORTABLE_CARD = 170.0' in kpi,'responsive KPI comfortable width floor is not the 9.0.49 compact 170px value')
...
need('CREATE TABLE IF NOT EXISTS lookup_code_alias' in master_migration and "~ '^GEN[0-9]+$'" in master_migration,'9.0.49 must migrate historical GENxxx codes and retain aliases')
need("VALUES('auth.selfRegistrationRole', 'SALES')" in master_migration,'9.0.49 must seed an upgrade-safe configurable registration-role setting')
