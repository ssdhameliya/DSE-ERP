...
require('BUILD_REVISION = "9.0.46"' in runtime_contract and 'APP_VERSION = "9.0.46"' in runtime_contract, '9.0.18 must publish one exact desktop/server version/build contract so stale backends are rejected')
...
require("DATE_TRUNC('month',CAST(NULLIF(TRIM(quotation_date),'') AS DATE))" in quotation_service
        ...
        or "DATE_TRUNC('month',dse_safe_date(q.quotation_date))" in quotation_service,
