...
# identity
need('<version>9.0.57</version>' in t('pom.xml') and '<dse.phase>9.0.57</dse.phase>' in t('pom.xml'),'root identity')
need('APP_VERSION = "9.0.57"' in t('shared/src/main/java/org/example/shared/RuntimeContract.java'),'runtime identity')
# session
...
for name in ['light-theme.css','dark-theme.css']:
    css=t('desktop/src/main/resources/css/'+name);need('session-timeout-countdown' in css and 'sales-order-stock-availability' in css,name+' missing 9.0.57 rules')
