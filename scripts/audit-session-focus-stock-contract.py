...
need("<version>9.0.74</version>" in t("pom.xml") and "<dse.phase>9.0.74</dse.phase>" in t("pom.xml"),"root identity")
need('APP_VERSION = "9.0.74"' in t("shared/src/main/java/org/example/shared/RuntimeContract.java"),"runtime identity")
...
for token in ["Category: ","HSN: ","Unit: ","GST: "]:
    need(token in sales,"Sale item search metadata missing "+token)
