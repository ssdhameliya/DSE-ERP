...
need('pom.xml','<version>9.0.40</version>','<maven.compiler.release>25</maven.compiler.release>')
need('mvnw','3.9.11')
...
need('.github/workflows/release.yml','./mvnw','audit-9.0.40-engineering-hardening.py')
...
need('desktop/src/main/java/org/example/update/ReleaseHighlights.java','if ("9.0.40".equals(version))','Engineering' if False else 'Maven Wrapper')

if FAIL:
    print('FAIL: 9.0.40 engineering hardening audit')
    for f in FAIL: print(' -',f)
    sys.exit(1)
print('PASS: 9.0.40 engineering hardening audit')
