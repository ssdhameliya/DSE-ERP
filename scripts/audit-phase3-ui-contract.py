...
check('viewport allocation protects actions centrally', 'ACTION_CONTROL_MIN_WIDTH = 118.0' in mgr and 'allocateDense(' in mgr)
check('Actions remain protected', 'ACTION_CONTROL_MIN_WIDTH = 118.0' in mgr)
check('constrained viewport-fill policy removes horizontal-scroll/right-filler dependence', 'CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN' in mgr)
...
check('table layout coalesces structural generations without VirtualFlow listeners', 'erp.table.dynamic-layout.generation' in mgr and 'runScheduledLayout(' in mgr and '.virtual-flow' not in mgr and 'SAMPLED_CONTENT_WIDTH' not in mgr)
