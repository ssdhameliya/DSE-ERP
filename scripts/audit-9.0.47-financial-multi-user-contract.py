#!/usr/bin/env python3
"""DSE ERP 9.0.47 financial integrity and multi-user hardening contract."""
...
need("scaled(value, 2" in calc and "RoundingMode.HALF_UP" in calc, "canonical 2dp HALF_UP money contract missing")
need("scaled(value, 4" in calc and "RoundingMode.HALF_UP" in calc, "canonical 4dp quantity/unit-cost contract missing")
...
need("quantity(input.quantity())" in calc and "money(finiteNonNegative(input.rate()))" in calc, "line calculation must normalize quantity and rate before calculation")
...
for token in (
    ...
    need(token in migration, f"9.0.47 migration missing: {token}")
need("V9_0_47__financial_multi_user_hardening" in runner, "9.0.47 migration is not registered at startup")
