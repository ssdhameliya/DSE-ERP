...
need('pom.xml','<version>9.0.45</version>','<maven.compiler.release>25</maven.compiler.release>')
need('mvnw','3.9.11')
...
need('.github/workflows/release.yml','./mvnw','audit-9.0.45-engineering-hardening.py')
need('.github/workflows/ci.yml','audit-9.0.45-ui-behavior-freeze.py','audit-9.0.45-phase2-two-theme-contract.py','audit-9.0.45-phase3-semantic-ui-contract.py','audit-9.0.45-phase4-responsive-kpi-contract.py','audit-9.0.45-phase5-dynamic-table-contract.py','audit-9.0.45-phase6-controller-cleanup-contract.py','audit-9.0.45-phase7-production-clean-contract.py','audit-9.0.45-phase8-ui-stabilization-contract.py')
need('.github/workflows/release.yml','audit-9.0.45-ui-behavior-freeze.py','audit-9.0.45-phase2-two-theme-contract.py','audit-9.0.45-phase3-semantic-ui-contract.py','audit-9.0.45-phase4-responsive-kpi-contract.py','audit-9.0.45-phase5-dynamic-table-contract.py','audit-9.0.45-phase6-controller-cleanup-contract.py','audit-9.0.45-phase7-production-clean-contract.py','audit-9.0.45-phase8-ui-stabilization-contract.py')
need('.github/workflows/ci.yml','audit-9.0.45-ui-enhancement-contract.py')
need('.github/workflows/release.yml','audit-9.0.45-ui-enhancement-contract.py')
need('scripts/audit-9.0.45-ui-behavior-freeze.py','UI_BEHAVIOR_FREEZE_OK','protected_navigation_hashes')
need('scripts/audit-9.0.45-phase2-two-theme-contract.py','PHASE2_TWO_THEME_OK','EXPECTED_CSS','scene.getStylesheets().setAll(')
need('scripts/audit-9.0.45-phase3-semantic-ui-contract.py','PHASE3_SEMANTIC_UI_OK','UiSemanticRegistry.headerSemantic')
need('scripts/audit-9.0.45-phase4-responsive-kpi-contract.py','PHASE4_RESPONSIVE_KPI_OK','ResponsiveKpiLayoutManager','responsiveColumnCount')
need('scripts/audit-9.0.45-phase5-dynamic-table-contract.py','PHASE5_DYNAMIC_TABLE_OK','DynamicTableLayoutManager','width_attrs=0')
need('scripts/audit-9.0.45-phase6-controller-cleanup-contract.py','PHASE6_CONTROLLER_CLEANUP_OK','SettingsAssetService','static_fxml_header_calls=0')
need('scripts/audit-9.0.45-phase7-production-clean-contract.py','PHASE7_PRODUCTION_CLEAN_OK','EXPECTED_CSS','junk=0 trailing_whitespace=0')
need('scripts/audit-9.0.45-phase8-ui-stabilization-contract.py','PHASE8_UI_STABILIZATION_OK','renderedCellControlWidth','yellow_button_surfaces=0')
need('scripts/audit-9.0.45-ui-enhancement-contract.py','UI_ENHANCEMENT_9_0_45_OK','reporting=table-first','action=icon+text','bank_kpi=8x1')
need('scripts/audit-9.0.45-final-contract.py','FINAL_9_0_45_OK','schedule_pdf_xlsx=both','full_runtime=yes')
...
need('scripts/ui-behavior-freeze-9.0.45.json','DSE ERP 9.0.45 approved UI/behavior freeze')
...
need('desktop/src/main/java/org/example/update/ReleaseHighlights.java','if ("9.0.45".equals(version))','Engineering' if False else 'Maven Wrapper')

if FAIL:
    print('FAIL: 9.0.45 engineering hardening audit')
    for f in FAIL: print(' -',f)
    sys.exit(1)
print('PASS: 9.0.45 engineering hardening audit')
