...
req('<version>9.0.77</version>' in root_pom and '<dse.phase>9.0.77</dse.phase>' in root_pom,
    'root Maven release identity must be 9.0.77')
for name,pom in [('server',server_pom),('desktop',desktop_pom),('shared',shared_pom)]:
    req('<version>9.0.77</version>' in pom, f'{name} parent version must be 9.0.77')
...
req('version=9.0.77' in app_version and 'DEFAULT_VERSION="9.0.77"' in update,
    'desktop resource/updater identity must be 9.0.77')
if runtime_manifest:
    req('runtime.phase=9.0.77' in runtime_manifest, 'bundled runtime phase must be 9.0.77')
req('DSE ERP 9.0.77 - DEVELOPMENT / INTELLIJ ONLY' in run_bat,
    'IntelliJ launcher banner must be 9.0.77')
req('DSE ERP 9.0.77 - PRODUCTION WINDOWS BUILD' in build_bat,
    'production build banner must be 9.0.77')
req('DSE ERP 9.0.77 uses application-managed PostgreSQL.' in postgres_bat,
    'PostgreSQL launcher banner must be 9.0.77')
req('fx:id="lblCurrentVersion" text="9.0.77"' in safe_rollback,
    'Safe Rollback current-version fallback must be 9.0.77')
...
print('CORRECTIVE_RUNTIME_UI_CONTRACT_OK version=9.0.77')
