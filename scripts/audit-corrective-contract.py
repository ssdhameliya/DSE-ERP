...
req('if (sales) ImportDocumentPolicy.requireReference(referenceFormats, "REF_SALES", invoice' in importer,
    'Sales import invoice format validation must remain enforced through extracted ImportDocumentPolicy')
...
# v9.0.12 runtime/UI regressions reported from real desktop testing.
...
req('if (taxType.isBlank()) taxType = "GST";' in sales_controller,
    'Sales create screen must tolerate the blank GST ComboBox bootstrap window')
req('safeValue(cmbGstType.getValue(),"GST")' in purchase_controller,
    'Purchase create screen must tolerate the blank GST ComboBox bootstrap window')
...
req('V9_0_11__finance_runtime_repair' in migration_runner,
    'runtime migration runner must execute the v9.0.12 Finance repair before READY')
for required in ['finance_register", "account_name','finance_register", "bill_path','finance_register", "reconciled',
                 'bank_reconciliation_allocation", "finance_entry_id','bank_reconciliation_allocation", "rounding_adjustment']:
    req(required in migration_runner, f'runtime schema verification missing: {required}')

req('userMessage(String area, int status, String body)' in api_runtime and 'logHttpFailure' in api_runtime,
    'API runtime must separate user-facing HTTP messages from technical logging')
for name,client in [('operations',ops_client),('bank statement',bank_client),('master',master_client)]:
    req('ApiRuntime.userMessage' in client and 'ApiRuntime.logHttpFailure' in client,
        f'{name} API client must use centralized user-facing error translation')
req('userFacingError' in modern_dialog and 'empty string' in modern_dialog.lower(),
    'modern dialogs must translate raw/empty technical exception messages')
req('userMessage(e)' in bank_expense_controller and 'parseEntryDate' in bank_expense_controller,
    'Bank/Expense screen must sanitize save errors and tolerate legacy dates')

req('erp.icon.skip' in permission_matrix and 'setContentDisplay(ContentDisplay.TEXT_ONLY)' in permission_matrix
    and ' + " of " + ' in permission_matrix,
    'Permission Matrix Special column must remain readable text instead of icon-only decoration')
req('new ScrollPane(content)' in dashboard_controller and 'Screen.getScreensForRectangle' in dashboard_controller
    and 'setPrefViewportHeight(520)' in dashboard_controller,
    'Keyboard Shortcuts dialog must be scrollable and constrained to the active screen')

req('Auto Map Fields' in pdf_fxml and 'Browse / Drag ERP Fields' in pdf_fxml
    and pdf_fxml.count('<FlowPane') >= 2,
    'PDF Studio must expose mapping clearly and wrap toolbar actions rather than ellipsize them')
req('loadPreviewData(sample, false)' in pdf_controller and 'analyzeMapping(false)' in pdf_controller,
    'opening a PDF Studio template must not auto-materialize mappings')
req('new Label("IMAGE")' not in pdf_controller and 'new Label(region.kind())' not in pdf_controller,
    'detected PDF image/vector hit regions must not draw permanent type labels over the source PDF')
...
req('<version>9.0.79</version>' in root_pom and '<dse.phase>9.0.79</dse.phase>' in root_pom,
    'root Maven release identity must be 9.0.79')
for name,pom in [('server',server_pom),('desktop',desktop_pom),('shared',shared_pom)]:
    req('<version>9.0.79</version>' in pom, f'{name} parent version must be 9.0.79')
req('APP_VERSION = "9.0.79"' in runtime and 'BUILD_REVISION = "9.0.79"' in runtime,
    'shared runtime identity must be 9.0.79')
req('dse.app.version=9.0.79' in props and 'dse.build.revision=9.0.79' in props,
    'server runtime identity must be 9.0.79')
req('version=9.0.79' in app_version and 'DEFAULT_VERSION="9.0.79"' in update,
    'desktop resource/updater identity must be 9.0.79')
if runtime_manifest:
    req('runtime.phase=9.0.79' in runtime_manifest, 'bundled runtime phase must be 9.0.79')
req('DSE ERP 9.0.79 - DEVELOPMENT / INTELLIJ ONLY' in run_bat,
    'IntelliJ launcher banner must be 9.0.79')
req('DSE ERP 9.0.79 - PRODUCTION WINDOWS BUILD' in build_bat,
    'production build banner must be 9.0.79')
req('DSE ERP 9.0.79 uses application-managed PostgreSQL.' in postgres_bat,
    'PostgreSQL launcher banner must be 9.0.79')
req('fx:id="lblCurrentVersion" text="9.0.79"' in safe_rollback,
    'Safe Rollback current-version fallback must be 9.0.79')
...
print('CORRECTIVE_RUNTIME_UI_CONTRACT_OK version=9.0.79')
