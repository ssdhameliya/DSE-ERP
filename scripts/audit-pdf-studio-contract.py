...
server_json_service = text('server/src/main/java/org/example/documentstudio/service/ErpDocumentJsonService.java')
...
builtin_template_json = text('desktop/src/main/resources/documentstudio/defaults/sales-invoice-jasvi-template.json')
...
req(server_json_service == json_service, 'server and desktop PDF Studio ERP JSON contracts must remain byte-identical to prevent runtime field drift')
req('alias(root, data, "document.poNumber", "sales.orderNo")' in json_service and 'defaultText(root, "document.poNumber", "N/A")' in json_service and 'defaultText(root, "document.poDate", "N/A")' in json_service, 'Sales PDF Studio PO number/date mapping must use orderNo and represent missing PO values as N/A')
...
req('FLOW_FIXED' in builtin_installer and 'sales-invoice-jasvi-runtime.pdf' in builtin_installer and 'sales-invoice-jasvi-template.json' in builtin_installer and ('pub.setDefaultTemplate(false)' in builtin_installer) and ('pub.setRuntimeEnabled(false)' in builtin_installer) and ('PdfStudioRemoteStore.publish' in builtin_installer), 'built-in Sales Studio template must install from centralized metadata as a non-active published candidate and remain server-shareable')
...
req('private static final int RELEASE_VERSION = 16;' in builtin_installer and 'sales-invoice-jasvi-template.json' in builtin_installer and 'PdfStudioTemplatePackageService.normalizeMappingMetadata' in builtin_installer and 'double[] xs' not in builtin_installer and '24.23' not in builtin_installer and '401.85' not in builtin_installer, 'current built-in Sales template must load concrete geometry only from template metadata and normalize it through the universal detector')
...
req('\"pageRule\": \"INTERMEDIATE\"' in builtin_template_json and 'case "INTERMEDIATE"' in renderer, 'multi-page PDF Studio Sales output must support final-only closing-stack suppression')
req('totals.breakdownLabels' in builtin_template_json and 'totals.breakdownAmounts' in builtin_template_json and 'DYNAMIC_FINANCIAL_SUMMARY' in builtin_template_json and ('financialBreakdownLabels' in json_service) and ('financialBreakdownAmounts' in json_service) and ('totals.chargeLabel' in json_service) and ('totals.chargesAmount' in json_service), 'fixed Sales template must render Standard-compatible zero/one/multiple charge breakdown rows while retaining compatibility aliases')
