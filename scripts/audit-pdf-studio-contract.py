...
req('private static final int RELEASE_VERSION = 5;' in builtin_installer
    ...
    '9.0.76 built-in Sales template must migrate Billing/Delivery GSTIN to the corrected source-PDF baseline')
...
req('APP_VERSION = "9.0.76"' in runtime and 'BUILD_REVISION = "9.0.76"' in runtime,
    'desktop/shared runtime identity must be 9.0.76')
req('dse.app.version=9.0.76' in props and 'dse.build.revision=9.0.76' in props,
    'server runtime identity must be 9.0.76')
req('<artifactId>dse-erp-parent</artifactId>\n  <version>9.0.76</version>' in root_pom and '<dse.phase>9.0.76</dse.phase>' in root_pom,
    'root Maven application version and phase must be 9.0.76')
for name, pom in [('shared', shared_pom), ('server', server_pom), ('desktop', desktop_pom)]:
    req('<artifactId>dse-erp-parent</artifactId>' in pom and '<version>9.0.76</version>' in pom,
        f'{name} Maven parent version must be 9.0.76')
req('version=9.0.76' in app_version, 'desktop app-version.properties must be 9.0.76')
req('DEFAULT_VERSION="9.0.76"' in update_service, 'update fallback version must be 9.0.76')
req('runtime.phase=9.0.76' in runtime_manifest, 'runtime identity manifest phase must be 9.0.76')
req('DSE ERP 9.0.76 - DEVELOPMENT / INTELLIJ ONLY' in run_bat,
    'development launcher banner must be 9.0.76')
req('DSE ERP 9.0.76 - PRODUCTION WINDOWS BUILD' in build_bat,
    'production Windows launcher banner must be 9.0.76')
req('DSE ERP 9.0.76 uses application-managed PostgreSQL.' in postgres_bat,
    'PostgreSQL launcher banner must be 9.0.76')

print('PDF_STUDIO_CONTRACT_OK version=9.0.76')
